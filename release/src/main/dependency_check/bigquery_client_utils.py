...
class BigQueryClientUtils:
  ...
  def query_dep_release_date(self, dep, version):
    ...
    WHERE package_name={3} and version = {4}""".format(self.project_id, self.dataset_id, self.table_id, dep, version)
  ...
  def add_dep_to_table(self, dep, version, release_date, is_current_using=False):
    ...
    self.bigquery_client.insert_rows(self.table_ref, dep_to_insert)
