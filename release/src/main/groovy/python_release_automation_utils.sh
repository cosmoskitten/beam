...
run_mobilegaming_pubsub_publish() {
    scores=("user7_AlmondMarmot,AlmondMarmot,9"
    ...
    do
        ...
        date_readable="$(date +%Y-%m-%d) $(date +"%T").000"
        gcloud pubsub topics publish $MOBILE_GAMING_PUBSUB_TOPIC --attribute "timestamp_ms"="$time_ms" --message "${scores[$i]},$time_ms,$date_readable"
    done
}
