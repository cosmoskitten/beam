...
t.run("bq rm -f -t python_leader_board_test.leaderboard_user")
t.run("bq rm -f -t python_leader_board_test.leaderboard_team")
...
injectorCmd.append("mvn compile exec:java ")
    ...
    .append("-Dexec.args=\"${t.gcpProject()} python-leaderboard-topic none\"")
...
cmd.append("mvn compile exec:java ")
        ...
        .append("""-Dexec.args=\"--project=${t.gcpProject()}
             ...
             --output=gs://gs://${t.gcsBucket()}/${t.gcsBucketSubDirectory()}/java-leaderboard-result.txt\" """)
        .append("-Pdirect-runner")
t.run(cmd.toString())
t.run("sleep 800")
...
t.run "bq query SELECT table_id FROM ${bqDataset()}.__TABLES_SUMMARY__"
t.see "leaderboard_user"
t.see "leaderboard_team"

// t.run """bq query --batch "SELECT user FROM [google.com:dataflow-streaming:python_leader_board_test.leaderboard_user] LIMIT 100\""""
// t.seeOneOf ***
