...
t.intent("Running: GameStats example on DirectRunner")
t.run("bq rm -f -t ${t.bqDataset()}.gamestats_DirectRunner_sessions")
t.run("bq rm -f -t ${t.bqDataset()}.gamestats_DirectRunner_team")
InjectorThread = Thread.start() {
    t.run(MobileGamingJavaUtils.generateCommand("Injector", "",t))
}

def GameStatsThread = Thread.start() {
    t.run(MobileGamingJavaUtils.generateCommand("GameStats", "DirectRunner", t))
}
...
t.run "bq query SELECT table_id FROM ${t.bqDataset()}.__TABLES_SUMMARY__"
t.see "gamestats_DirectRunner_sessions"
t.see "gamestats_DirectRunner_team"
t.run """bq query --batch "SELECT team FROM [${t.gcpProject()}:${t.bqDataset()}.gamestats_DirectRunner_team] LIMIT 10\""""
t.seeOneOf(MobileGamingJavaUtils.COLORS)
t.intent("SUCCEED: GameStats successfully run on DirectRunner.")
