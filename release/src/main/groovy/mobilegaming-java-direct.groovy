...
t.run("gcloud components update")
...
while((int)(System.currentTimeMillis() - startTime)/1000 < MobileGamingJavaUtils.EXECUTION_TIMEOUT) {
    t.run "bq query SELECT table_id FROM ${t.bqDataset()}.__TABLES_SUMMARY__"
    if(t.seeKeyWord("leaderboard_DirectRunner_user") && t.seeKeyWord("leaderboard_DirectRunner_team")){
        t.run """bq query --batch "SELECT user FROM [${t.gcpProject()}:${t.bqDataset()}.leaderboard_DirectRunner_user] LIMIT 10\""""
        if(t.seeKeyWord("AlmondMarmot")){
            isSucceess = true
            break
        }
    }
...
if(!isSucceess){
    t._error("FAILED: Did not find expected results from LeaderBoard outputs")
}
