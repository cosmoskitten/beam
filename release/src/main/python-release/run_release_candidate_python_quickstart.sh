...
CANDIDATE_URL=$RC_STAGING_URL
BEAM_PYTHON_SDK=$BEAM_PYTHON_SDK_ZIP
ASC_FILE_NAME=$TAR_ASC_FILE_NAME
SHA512_FILE_NAME=$TAR_SHA512_FILE_NAME
...
#######################################
# Download files from RC staging location
# Globals:
#   CANDIDATE_URL, SHA512_FILE_NAME,
#   ASC_FILE_NAME, BEAM_PYTHON_SDK
# Arguments:
#   None
#######################################
function download_files(){
  wget $CANDIDATE_URL$SHA512_FILE_NAME
...
function run_release_candidate_python_quickstart(){
  ...
  if [[ $1 = *"wheel"* ]]; then
    CANDIDATE_URL=$RC_STAGING_URL"python-wheels/"
    BEAM_PYTHON_SDK=$BEAM_PYTHON_SDK_WHL
    ASC_FILE_NAME=$WHL_ASC_FILE_NAME
    SHA512_FILE_NAME=$WHL_SHA512_FILE_NAME
  fi
  download_files
  verify_hash
  install_sdk
