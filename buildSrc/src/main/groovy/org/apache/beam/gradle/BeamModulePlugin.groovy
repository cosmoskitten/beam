...
class BeamModulePlugin implements Plugin<Project> {
  ...
  void apply(Project project) {
    ...
    project.ext.applyJavaNature = {
      ...
      def packageTests = project.task(name: 'packageTests', type: Jar) {
        classifier = 'tests'
        from sourceSets.test.output
      }
      project.artifacts.archives packageTests
      ...
      def shadowJar = project.shadowJar({
        classifier = "shaded"
        mergeServiceFiles()
        into("META-INF/") {
      ...
      def shadowTestJar = project.tasks.create(name: 'shadowTestJar', type: ShadowJar, {
      ...
      project.artifacts.shadow shadowJar
      project.artifacts.shadowTest shadowTestJar
...
''')
    ...
    project.ext.enableJavaPerformanceTesting = {
      ...
      project.task(name: 'integrationTest', type: Test) {
    ...
    project.ext.provideIntegrationTestingDependencies = {
      ...
      project.task(name: 'packageIntegrationTests', type: Jar) {
    ...
    project.ext.createPerformanceTestHarness = {
      ...
      project.task(type: Exec, name: 'performanceTest') {
    ...
    project.ext.createJavaExamplesArchetypeValidationTask = {
      ...
      project.task(name: taskName, dependsOn: ':release:classes', type: JavaExec) {
        group = "Verification"
