...
class VendorJavaPlugin implements Plugin<Project> {
  ...
  static class VendorJavaPluginExtension {
    String dependency
  ...
  void apply(Project project) {
    ...
    def extension = project.extensions.create('vendorJava', VendorJavaPluginExtension)
    ...
    project.ext.doVendorJava = {
      ...
      project.dependencies { compile "${extension.dependency}" }

      project.shadowJar {
        extension.packagesToRelocate.each { srcNamespace ->
          relocate(srcNamespace, "${extension.intoPackage}.${srcNamespace}")
        }
        classifier = null
        mergeServiceFiles()
        zip64 true
