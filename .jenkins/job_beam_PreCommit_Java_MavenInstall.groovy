...
mavenJob('beam_PreCommit_Java_MavenInstall') {
  ...
  goals('-B -e -Prelease,include-runners,jenkins-precommit,direct-runner,dataflow-runner,spark-runner,flink-runner,apex-runner -DrepoToken=$COVERALLS_REPO_TOKEN -DpullRequest=$ghprbPullId help:effective-settings clean install jacoco:report-aggregate coveralls:report')
}
