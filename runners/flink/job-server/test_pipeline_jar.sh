...
docker images $PYTHON_CONTAINER_IMAGE:$PYTHON_CONTAINER_TAG | grep $PYTHON_CONTAINER_TAG
...
(python -c "$PIPELINE_PY" \
  ...
  --environment_config=$PYTHON_CONTAINER_IMAGE:$PYTHON_CONTAINER_TAG \
