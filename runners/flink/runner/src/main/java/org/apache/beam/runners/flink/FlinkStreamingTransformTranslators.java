...
class FlinkStreamingTransformTranslators {
  ...
  public static FlinkStreamingPipelineTranslator.StreamTransformTranslator<?> getTranslator(
    ...
    if (transform instanceof Write) {
      Sink sink = ((Write) transform).getSink();
      return sink instanceof UnboundedFlinkSink ? TRANSLATORS.get(transform.getClass()) : null;
    } else {
  ...
  private static class WriteSinkStreamingTranslator<T>
    ...
    public void translateNode(Write<T> transform, FlinkStreamingTranslationContext context) {
      ...
      checkArgument(sink instanceof UnboundedFlinkSink);
