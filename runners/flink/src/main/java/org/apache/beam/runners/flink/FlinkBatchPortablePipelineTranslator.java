...
public class FlinkBatchPortablePipelineTranslator
  ...
  private static <InputT> void translateView(
    ...
    DataSet<WindowedValue<InputT>> inputDataSet =
        context.getDataSetOrThrow(Iterables.getOnlyElement(transform.getInputsMap().values()));

    context.addDataSet(Iterables.getOnlyElement(transform.getOutputsMap().values()), inputDataSet);
  }
  ...
  private static <K, V> void translateReshuffle(
    ...
    DataSet<WindowedValue<KV<K, V>>> inputDataSet =
        context.getDataSetOrThrow(Iterables.getOnlyElement(transform.getInputsMap().values()));
    context.addDataSet(
        Iterables.getOnlyElement(transform.getOutputsMap().values()), inputDataSet.rebalance());
  }
  ...
  private static <InputT> void translateExecutableStage(
    ...
    MapPartitionOperator<WindowedValue<InputT>, RawUnionValue> taggedDataset =
        new MapPartitionOperator<>(
            inputDataSet, typeInformation, function, transform.getUniqueName());
  ...
  private static <K, V> void translateGroupByKey(
    ...
    context.addDataSet(
        Iterables.getOnlyElement(pTransform.getOutputsMap().values()), outputDataSet);
  }
  ...
  private static void translateImpulse(
    ...
    context.addDataSet(Iterables.getOnlyElement(pTransform.getOutputsMap().values()), dataSource);
  }
