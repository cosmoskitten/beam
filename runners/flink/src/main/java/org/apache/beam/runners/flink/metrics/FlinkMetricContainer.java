...
public class FlinkMetricContainer {
              ...
              distribution -> {
                metricsContainer
                    ...
                    .update(
                        ...
                        distribution.getMin(),
                        distribution.getMax());
                updateDistribution(key, distribution);
              },
              gauge -> {
                metricsContainer.getGauge(metricName).set(gauge.getValue());
                updateGauge(key, gauge);
              });
        });
  }
  ...
  private <T> void updateMetrics(
    ...
    for (MetricResult<T> metricResult : metricResults) {
      fn.accept(MetricKey.create(metricResult), metricResult.getAttempted());
    }
  }
