...
public class ReduceFnRunnerTest {
  ...
  public void testOnElementCombiningWithContext() throws Exception {
        ...
        .then(
            new Answer<Integer>() {
              ...
              public Integer answer(InvocationOnMock invocation) throws Throwable {
                ...
                assertTrue(startMs == 0 || startMs == 4);
                assertEquals(4, endMs - startMs);
                return 100 + (int) startMs;
              }
            });
    ...
    assertThat(
        tester.extractOutput(),
        contains(
            isSingleWindowedValue(equalTo(0 + 100), 1, 0, 2),
            isSingleWindowedValue(equalTo(0 + 1 + 100), 1, 0, 2),
            isSingleWindowedValue(equalTo(2 + 100), 3, 2, 4),
            isSingleWindowedValue(equalTo(2 + 3 + 100), 3, 2, 4),
            isSingleWindowedValue(equalTo(4 + 104), 5, 4, 6),
            isSingleWindowedValue(equalTo(4 + 5 + 104), 5, 4, 6),
            isSingleWindowedValue(equalTo(6 + 104), 7, 6, 8),
            isSingleWindowedValue(equalTo(6 + 7 + 104), 7, 6, 8)));
  }
  ...
  private static class SumAndVerifyContextFn
    ...
    private void verifyContext(Context c) {
      assertEquals(expectedValue, c.getPipelineOptions().as(TestOptions.class).getValue());
      assertThat(c.sideInput(view), greaterThan(0));
    }
