...
public class ReduceFnRunnerTest {
  ...
  abstract static class Action {
    @Nullable abstract Instant inputWatermark();
    @Nullable abstract List<Integer> timestamps();

    static Action processTimestamps(Integer... timestamps) {
      // See "How do I...?" below for nested classes.
      return new AutoValue_ReduceFnRunnerTest_Action(null, Arrays.asList(timestamps));
    }

    static Action advanceInputWatermark(Instant inputWatermark) {
      return new AutoValue_ReduceFnRunnerTest_Action(inputWatermark, null);
    }
  }
  ...
  // Performs the specified actions and verifies that the watermark hold is set correctly.
  public void mergingWatermarkHoldTestHelper(List<Action> configuration) throws Exception {
    ...
    Instant maxTs = new Instant(0);
    Instant watermark = new Instant(0);
    for (Action action : configuration) {
      if (action.timestamps() != null) {
        LOG.info("Injecting {}", action.timestamps());
        injectElements(tester, action.timestamps());
        int maxLocalTs = Ordering.natural().max(action.timestamps());
        if (maxLocalTs > maxTs.getMillis()) {
          maxTs = new Instant(maxLocalTs);
        }
      }
      if (action.inputWatermark() != null && action.inputWatermark().isAfter(watermark)) {
        watermark = action.inputWatermark();
        LOG.info("Advancing watermark to {}", watermark);
        tester.advanceInputWatermark(watermark);
      }
      Instant hold = tester.getWatermarkHold();
      if (hold != null) {
        assertThat(hold, greaterThanOrEqualTo(watermark));
        assertThat(watermark, lessThan(maxTs.plus(gapDuration)));
      }
    }
    if (new Instant(maxTs).plus(gapDuration).isAfter(watermark)) {
      watermark = maxTs.plus(gapDuration);
      tester.advanceInputWatermark(watermark);
    }
    LOG.info("Output {}", tester.extractOutput());
    assertThat(tester.getWatermarkHold(), nullValue());
    tester.advanceInputWatermark(watermark.plus(allowedLateness));
    assertThat(tester.getWatermarkHold(), nullValue());
  ...
  public void testMergingWatermarkHoldLateNewWindow() throws Exception {
    ...
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(1));
  ...
  public void testMergingWatermarkHoldLateNewWindowExtended() throws Exception {
    ...
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(1));
    actions.add(Action.processTimestamps(10));
    mergingWatermarkHoldTestHelper(actions);
  }
  ...
  public void testMergingWatermarkHoldLateNewWindowMerged() throws Exception {
    ...
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(1));
    actions.add(Action.processTimestamps(14));
    actions.add(Action.processTimestamps(6));
    mergingWatermarkHoldTestHelper(actions);
  }

  @Test
  public void testMergingWatermarkHoldLateNewWindowExtendedPastInputWatermark() throws Exception {
    LinkedList<Action> actions = new LinkedList<>();
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(25));
    actions.add(Action.processTimestamps(33));
    mergingWatermarkHoldTestHelper(actions);
  }

  @Test
  public void testMergingWatermarkHoldLateNewWindowExtendedPastInputWatermarkAndFurther()
      throws Exception {
    LinkedList<Action> actions = new LinkedList<>();
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(25));
    actions.add(Action.processTimestamps(33));
    actions.add(Action.processTimestamps(43));
    mergingWatermarkHoldTestHelper(actions);
  }
  ...
  public void testMergingWatermarkHoldExtendClosedWindow() throws Exception {
    LinkedList<Action> actions = new LinkedList<>();
    actions.add(Action.processTimestamps(11));
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(18));
    mergingWatermarkHoldTestHelper(actions);
  }

  @Test
  public void testMergingWatermarkHoldExtendClosedWindowAndMergeWithNew() throws Exception {
    LinkedList<Action> actions = new LinkedList<>();
    actions.add(Action.processTimestamps(11));
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(18));
    actions.add(Action.processTimestamps(41));
    actions.add(Action.processTimestamps(27, 33));
    mergingWatermarkHoldTestHelper(actions);
  }

  @Test
  public void testMergingWatermarkHoldLateNewWindowMergedWithOntime() throws Exception {
    LinkedList<Action> actions = new LinkedList<>();
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(29));
    actions.add(Action.processTimestamps(45));
    actions.add(Action.processTimestamps(36));
    mergingWatermarkHoldTestHelper(actions);
  }

  @Test
  public void testMergingWatermarkHoldLateNewWindowMergedWithOntimeSingleProcessing()
      throws Exception {
    LinkedList<Action> actions = new LinkedList<>();
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(29, 45, 36));
    mergingWatermarkHoldTestHelper(actions);
  }
  ...
  public void testMergingWatermarkHoldRepeatedMerging() throws Exception {
    ...
    actions.add(Action.advanceInputWatermark(new Instant(40)));
    actions.add(Action.processTimestamps(25));
    actions.add(Action.processTimestamps(42));
    actions.add(Action.processTimestamps(33, 21));
    actions.add(Action.advanceInputWatermark(new Instant(50)));
    actions.add(Action.processTimestamps(12));
    mergingWatermarkHoldTestHelper(actions);
  }
