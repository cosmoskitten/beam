...
public class OrFinallyReactorTest {
  ...
  public void testFireDeadline() throws Exception {
    ...
    assertEquals(new Instant(9),
        RepeatedlyReactor.forever(AfterWatermarkReactor.pastEndOfWindow())
        .getWatermarkThatGuaranteesFiring(window));
    assertEquals(new Instant(9), RepeatedlyReactor.forever(AfterWatermarkReactor.pastEndOfWindow())
        .orFinally(AfterPaneReactor.elementCountAtLeast(1))
        .getWatermarkThatGuaranteesFiring(window));
    assertEquals(new Instant(9), RepeatedlyReactor.forever(AfterPaneReactor.elementCountAtLeast(1))
        .orFinally(AfterWatermarkReactor.pastEndOfWindow())
        .getWatermarkThatGuaranteesFiring(window));
    assertEquals(new Instant(9),
        AfterPaneReactor.elementCountAtLeast(100)
    ...
    assertEquals(BoundedWindow.TIMESTAMP_MAX_VALUE,
        RepeatedlyReactor.forever(AfterPaneReactor.elementCountAtLeast(1))
        .orFinally(AfterPaneReactor.elementCountAtLeast(10))
        .getWatermarkThatGuaranteesFiring(window));
  }

  @Test
  public void testContinuation() throws Exception {
    OnceTrigger triggerA = AfterProcessingTimeReactor.pastFirstElementInPane();
    OnceTrigger triggerB = AfterWatermarkReactor.pastEndOfWindow();
    TriggerReactor aOrFinallyB = triggerA.orFinally(triggerB);
    TriggerReactor bOrFinallyA = triggerB.orFinally(triggerA);
    assertEquals(
  ...
  public void testToString() {
    TriggerReactor trigger = StubTriggerReactor.named("t1").orFinally(StubTriggerReactor.named("t2"));
