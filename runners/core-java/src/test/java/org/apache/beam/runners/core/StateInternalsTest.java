...
public abstract class StateInternalsTest {
  ...
  private static final StateTag<BagState<String>> STRING_BAG_ADDR1 =
      StateTags.bag("badStringBag", new BadStringUtf8Coder());
  ...
  public void testBagWithBadCoderEquality() throws Exception {
    // Ensure two instances of the bad coder are distinct
    assertThat(new BadStringUtf8Coder(), not(equalTo(new BadStringUtf8Coder())));
  ...
  private static class BadStringUtf8Coder extends Coder<String> {
