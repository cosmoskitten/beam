...
public class InMemoryTimerInternals implements TimerInternals {
  ...
  private NavigableSet<TimerData> queue(TimeDomain domain) {
    switch (domain) {
      case EVENT_TIME:
  ...
  public void setTimer(TimerData timerData) {
    ...
    @Nullable TimerData existing = existingTimers.get(timerData.getNamespace(), timerData.getTimerId());
    if (existing == null) {
      ...
      if (!timerData.getTimestamp().equals(existing.getTimestamp())) {
        NavigableSet<TimerData> timers = queue(timerData.getDomain());
  ...
  private TimerData removeNextTimer(Instant currentTime, TimeDomain domain) {
    NavigableSet<TimerData> queue = queue(domain);

    if (!queue.isEmpty() && currentTime.isAfter(queue.first().getTimestamp())) {
      TimerData timer = queue.first();
      existingTimers.remove(timer.getNamespace(), timer.getTimerId());
      queue.remove(timer);
      return timer;
    } else {
