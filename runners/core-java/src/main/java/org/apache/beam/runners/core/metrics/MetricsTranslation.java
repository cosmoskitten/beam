...
public abstract class MetricsTranslation {
  ...
  public static Map<String, Collection<BeamFnApi.Metrics.User>> metricUpdatesToProto(
    ...
    try {
      return metricUpdatesToProtoOrThrow(metricUpdates);
    } catch (ExecutionException exn) {
      throw new RuntimeException("Impossible error converting metric updates to proto", exn);
    }
  }

  private static Map<String, Collection<BeamFnApi.Metrics.User>> metricUpdatesToProtoOrThrow(
      MetricUpdates metricUpdates) throws ExecutionException {
    ...
    for (MetricUpdates.MetricUpdate<Long> counterUpdate : metricUpdates.counterUpdates()) {
      fnMetrics
          .get(counterUpdate.getKey().stepName())
          .add(
    ...
    for (MetricUpdates.MetricUpdate<GaugeData> gaugeUpdate : metricUpdates.gaugeUpdates()) {
      fnMetrics
          .get(gaugeUpdate.getKey().stepName())
          .add(
              BeamFnApi.Metrics.User.newBuilder()
    ...
    for (MetricUpdates.MetricUpdate<DistributionData> distributionUpdate :
      ...
      fnMetrics
          .get(distributionUpdate.getKey().stepName())
          .add(
              BeamFnApi.Metrics.User.newBuilder()
