...
public class SimpleMonitoringInfoBuilder {
  ...
  /**
   * Sets the urn of the MonitoringInfo to a proper user metric distribution URN for the given params.
   *
   * @param namespace
   * @param name
   */
  ...
  /** Sets the IntDistributionData of the DistributionData in the MonitoringInfo, and the appropriate type URN. */
  public SimpleMonitoringInfoBuilder setInt64DistributionValue(DistributionData data) {
    this.builder.getMetricBuilder().getDistributionDataBuilder().getIntDistributionDataBuilder()
        .setCount(data.count())
        .setSum(data.sum())
        .setMin(data.min())
        .setMax(data.max());
