...
public class SimpleDoFnRunner<InputT, OutputT> implements DoFnRunner<InputT, OutputT> {
  ...
  private class DoFnExtraContextFactory<InputT, OutputT>
    ...
    /** The state namespace for this context. Lazily initialized; will crash if
     * access when more than one window is present.
     */
    @Nullable
    private StateNamespace namespace;

    public DoFnExtraContextFactory(
        Collection<? extends BoundedWindow> windows,
        PaneInfo pane) {
    ...
    public State state(String stateId) {
      try {
      StateSpec<?, ?> spec = (StateSpec<?, ?>) signature.stateDeclarations().get(stateId).field()
          .get
          (fn);
      return stepContext.stateInternals().state(getNamespace(),
