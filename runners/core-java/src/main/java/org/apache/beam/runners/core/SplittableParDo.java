...
public class SplittableParDo<InputT, OutputT, RestrictionT>
  ...
  private static <InputT, OutputT, RestrictionT>
    ...
    PCollection<KeyedWorkItem<String, ElementAndRestriction<InputT, RestrictionT>>> keyedWorkItems =
        input
            ...
            .setCoder(splitCoder)
            // We need to explode windows before the GBK, so that the elements in KeyedWorkItem's
            // are each in a single window. This is necessary because ProcessFn sets timers, and
            // timers are namespaced to a single window and it should be the window of the input
            // element. See also: https://issues.apache.org/jira/browse/BEAM-1983
            // We need to also do this before assigning the unique key, because otherwise we'd end
            // up with multiple WindowedValue's per key which would make it non-unique.
            .apply(
                "Explode windows",
  ...
  /** A {@link DoFn} that forces each of its outputs to be in a single window. */
