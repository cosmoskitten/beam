                ...
                .setParameter(
                    Any.pack(
                        BytesValue.newBuilder()
                            .setValue(
                                ByteString.copyFrom(
                                    SerializableUtils.serializeToByteArray(sink)))
                            .build())))
        .build();
  }

  private static FileBasedSink<?> fromProto(SdkFunctionSpec sinkProto) throws IOException {


    byte[] serializedSink =
        sinkProto.getSpec().getParameter().unpack(BytesValue.class).toByteArray();

    return (FileBasedSink<?>) SerializableUtils.deserializeFromByteArray(
        serializedSink, FileBasedSink.class.getSimpleName());
  }

  public static <T> FileBasedSink<T> getSink(
      AppliedPTransform<PCollection<T>, PDone, PTransform<PCollection<T>, PDone>> transform)
      throws IOException {
    return (FileBasedSink<T>) fromProto(getWriteFilesPayload(transform).getSink());
  }
  ...
  static class WriteFilesTranslator implements TransformPayloadTranslator<WriteFiles<?>> {
    ...
    public FunctionSpec translate(
      ...
      return FunctionSpec.newBuilder()
          ...
          .setParameter(
              Any.pack(
                  WriteFilesPayload.newBuilder()
                      .setSink(toProto(transform.getTransform().getSink()))
                      .setWindowedWrites(transform.getTransform().isWindowedWrites())
                      .setRunnerDeterminedSharding(
                          transform.getTransform().getNumShards() == null
                              && transform.getTransform().getSharding() == null)
                      .build()))
          .build();
    }
  }
