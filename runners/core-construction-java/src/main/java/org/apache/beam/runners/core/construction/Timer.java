 ...
 *
 * <p>Note that this implementation is specifically used during execution within runners and
 * inside the SDK harness. The {@link org.apache.beam.sdk.state.Timer} represents the current
 * user facing API. Consider consolidating the two once
 * {@link org.apache.beam.runners.core.TimerInternals} is no longer the way in which users
 * construct/interact with timers.
 */
@AutoValue
public abstract class Timer<T> {
   ...
   *
   * <p>The time is relative to the time domain defined in the
   * {@link org.apache.beam.model.pipeline.v1.RunnerApi.TimerSpec} that is associated with this
   * timer.
   */
   ...
   *
   * <p>This coder is inexpensive for size estimation of elements if the payload coder
   * is inexpensive for size estimation.
   */
