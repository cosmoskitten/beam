...
public class UnboundedReadFromBoundedSource<T> extends PTransform<PBegin, PCollection<T>> {
  ...
  public static class BoundedToUnboundedSourceAdapter<T>
    ...
    public BoundedToUnboundedSourceAdapter(ArrayDeque<BoundedSource<T>> boundedSources) {
      ...
      this.checkpointCoder = new CheckpointCoder<>(boundedSources.peek().getDefaultOutputCoder());
    }
    ...
    public Coder<T> getOutputCoder() {
      return boundedSources.poll().getOutputCoder();
    }
        ...
        if (!residualElements.done()) {
          ...
          checkpoint =
              new Checkpoint<>(
                  ...
                  residualSources == null ? null : residualSources.getSources());
        } else if (residualSources != null) {
          checkpoint = residualSources.getCheckpointMark();
        } else {
          checkpoint = new Checkpoint<>(null /* residualElements */, null /* residualSource */);
        }
    ...
    private class ResidualSources {
      ...
      private BoundedSource<T> currentResidualSource;
      ...
      Checkpoint<T> getCheckpointMark() {
        if (reader == null) {
          ...
          if (residualSplit != null) residualSources.addFirst(residualSplit);
