...
public class WindowIntoTranslation {
  ...
  public static WindowFn<?, ?> getWindowFn(AppliedPTransform<?, ?, ?> application) {
    ...
    try {
      windowIntoPayload = transformProto.getSpec().getParameter().unpack(WindowIntoPayload.class);
    ...
    return WindowingStrategyTranslation.windowFnFromProto(windowIntoPayload.getWindowFn());
  }
