...
public class QueryablePipeline {
  ...
  public Collection<KeyedStateReference> getKeyedStates(PTransformNode transform) {
    return getLocalKeyedStateNames(transform.getTransform())
        .stream()
        .map(
            localName -> {
              String transformId = transform.getId();
              ...
              String collectionId = Iterables.getOnlyElement(
                  Sets.difference(
  ...
  private Set<String> getLocalKeyedStateNames(PTransform transform) {
    if (PTransformTranslation.PAR_DO_TRANSFORM_URN.equals(transform.getSpec().getUrn())) {
      try {
