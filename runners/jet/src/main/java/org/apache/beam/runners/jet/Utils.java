...
public class Utils {
  ...
  public static String getTupleTagId(PValue value) {
    ...
    if (expansion.size() != 1) {
      throw new RuntimeException(); // todo: Houston, we have a problem!
    }
  ...
  static WindowingStrategy<?, ?> getWindowingStrategy(AppliedPTransform<?, ?, ?> appliedTransform) {
    ...
    PValue taggedValue = outputs.entrySet().iterator().next().getValue();
    checkState(
  ...
  public static <T> byte[] encodeWindowedValue(
      WindowedValue<T> windowedValue, Coder coder, ByteArrayOutputStream baos) {
    try {
      baos.reset();
