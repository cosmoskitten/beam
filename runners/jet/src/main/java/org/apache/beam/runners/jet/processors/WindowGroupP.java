...
public class WindowGroupP<T, K> extends AbstractProcessor {
  ...
  private WindowGroupP(
    ...
    // System.out.println(WindowGroupP.class.getSimpleName() + " CREATE ownerId = " + ownerId);
  ...
  protected boolean tryProcess(int ordinal, @Nonnull Object item) {
    // System.out.println(WindowGroupP.class.getSimpleName() + " UPDATE ownerId = " + ownerId + ",
    // item = " + item); //useful for debugging
    assert ordinal == 0;
  ...
  private boolean flush(long timestamp, boolean appendWatermark) {
    if (flushTraverser == null) {
      Iterator<Entry<BoundedWindow, Map<K, List<WindowedValue<KV<K, T>>>>>> iterator =
      ...
      flushTraverser =
          windowTraverser
              .flatMap(
                  mainEntry ->
                      Traversers.traverseStream(mainEntry.getValue().entrySet().stream())
                          .map(
                              e ->
                                  (Object)
                                      createOutput(e.getKey(), mainEntry.getKey(), e.getValue())))
              ...
              windowingStrategy.getWindowFn().new MergeContext() {
                ...
                public void merge(Collection<BoundedWindow> windowsFrom, BoundedWindow windowTo) {
                  ...
                  for (BoundedWindow windowFrom : windowsFrom) {
                    ...
                    Map<K, List<WindowedValue<KV<K, T>>>> windowFromData =
                        windowToKeyToList.get(windowFrom);
  ...
  private byte[] createOutput(
      K key, BoundedWindow window, List<WindowedValue<KV<K, T>>> windowedValues) {
    ...
    for (WindowedValue<KV<K, T>> windowedValue : windowedValues) {
      if (!PaneInfo.NO_FIRING.equals(windowedValue.getPane())) {
    ...
    WindowedValue<KV<K, List<T>>> windowedValue =
        WindowedValue.of(KV.of(key, values), timestamp, window, PaneInfo.NO_FIRING);
    return Utils.encodeWindowedValue(windowedValue, outputCoder, baos);
  }

  /**
   * A utility to iterate windows in windowToKeyToList but only those that contain the given key
   * with minimum garbage possible.
   *
   * <p>Only the {@link #iterator()} method is ever used on this class.
   */
  private static class WindowsForKeyCollection<K, T> extends AbstractCollection<BoundedWindow> {
    private final Map<BoundedWindow, Map<K, List<WindowedValue<KV<K, T>>>>> windowToKeyToList;
    private final K key;

    WindowsForKeyCollection(
        Map<BoundedWindow, Map<K, List<WindowedValue<KV<K, T>>>>> windowToKeyToList, K key) {
