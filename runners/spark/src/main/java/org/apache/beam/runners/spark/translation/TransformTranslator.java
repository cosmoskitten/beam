...
public final class TransformTranslator {
  ...
  private static <InputT, ArrayT, OutputT> TransformEvaluator<Combine.Globally<InputT, OutputT>>
  combineGlobally() {
    return new TransformEvaluator<Combine.Globally<InputT, OutputT>>() {
      ...
      public void evaluate(Combine.Globally<InputT, OutputT> transform, EvaluationContext context) {
        final Combine.CombineFn<InputT, ArrayT, OutputT> globally =
        ...
        /*ArrayT*/ byte[] acc = inRddBytes.aggregate(
            CoderHelpers.toByteArray(globally.createAccumulator(), aCoder),
            new Function2</*ArrayT*/ byte[], /*InputT*/ byte[], /*ArrayT*/ byte[]>() {
              @Override
              public /*ArrayT*/ byte[] call(/*ArrayT*/ byte[] ab, /*InputT*/ byte[] ib)
                  throws Exception {
                ArrayT a = CoderHelpers.fromByteArray(ab, aCoder);
                InputT i = CoderHelpers.fromByteArray(ib, iCoder);
                return CoderHelpers.toByteArray(globally.addInput(a, i), aCoder);
              }
            },
            new Function2</*ArrayT*/ byte[], /*ArrayT*/ byte[], /*ArrayT*/ byte[]>() {
              @Override
              public /*ArrayT*/ byte[] call(/*ArrayT*/ byte[] a1b, /*ArrayT*/ byte[] a2b)
                  throws Exception {
                ArrayT a1 = CoderHelpers.fromByteArray(a1b, aCoder);
                ArrayT a2 = CoderHelpers.fromByteArray(a2b, aCoder);
                // don't use Guava's ImmutableList.of as values may be null
                List<ArrayT> accumulators = Collections.unmodifiableList(Arrays.asList(a1, a2));
                ArrayT merged = globally.mergeAccumulators(accumulators);
  ...
  private static <K, ValueInputT, ValueArrayT, ValueOutputT>
  TransformEvaluator<Combine.PerKey<K, ValueInputT, ValueOutputT>> combinePerKey() {
    return new TransformEvaluator<Combine.PerKey<K, ValueInputT, ValueOutputT>>() {
      @Override
      public void evaluate(Combine.PerKey<K, ValueInputT, ValueOutputT>
                               transform, EvaluationContext context) {
        final Combine.KeyedCombineFn<K, ValueInputT, ValueArrayT, ValueOutputT> keyed =
            COMBINE_PERKEY_FG.get("fn", transform);
        @SuppressWarnings("unchecked")
        JavaRDDLike<WindowedValue<KV<K, ValueInputT>>, ?> inRdd =
            (JavaRDDLike<WindowedValue<KV<K, ValueInputT>>, ?>) context.getInputRDD(transform);

        @SuppressWarnings("unchecked")
        KvCoder<K, ValueInputT> inputCoder = (KvCoder<K, ValueInputT>)
            context.getInput(transform).getCoder();
        Coder<K> keyCoder = inputCoder.getKeyCoder();
        Coder<ValueInputT> viCoder = inputCoder.getValueCoder();
        Coder<ValueArrayT> vaCoder;
        try {
        ...
        Coder<KV<K, ValueInputT>> kviCoder = KvCoder.of(keyCoder, viCoder);
        Coder<KV<K, ValueArrayT>> kvaCoder = KvCoder.of(keyCoder, vaCoder);
        ...
        // require the key in addition to the InputT's and ValueArrayT's being merged/accumulated.
        ...
        JavaPairRDD<WindowedValue<K>, WindowedValue<KV<K, ValueInputT>>> inRddDuplicatedKeyPair =
            inRdd.flatMapToPair(
                new PairFlatMapFunction<WindowedValue<KV<K, ValueInputT>>, WindowedValue<K>,
                    WindowedValue<KV<K, ValueInputT>>>() {
                  @Override
                  public Iterable<Tuple2<WindowedValue<K>,
                      WindowedValue<KV<K, ValueInputT>>>>
                  call(WindowedValue<KV<K, ValueInputT>> kv) {
                      List<Tuple2<WindowedValue<K>,
                          WindowedValue<KV<K, ValueInputT>>>> tuple2s =
        ...
        final WindowedValue.FullWindowedValueCoder<KV<K, ValueInputT>> wkviCoder =
                WindowedValue.FullWindowedValueCoder.of(kviCoder,
                context.getInput(transform).getWindowingStrategy().getWindowFn().windowCoder());
        final WindowedValue.FullWindowedValueCoder<KV<K, ValueArrayT>> wkvaCoder =
        ...
        // The output of combineByKey will be "ValueArrayT" (accumulator)
        ...
        JavaPairRDD</*K*/ ByteArray, /*KV<K, ValueArrayT>*/ byte[]> accumulatedBytes =
            inRddDuplicatedKeyPairBytes.combineByKey(
            new Function</*KV<K, InputT>*/ byte[], /*KV<K, ValueArrayT>*/ byte[]>() {
              @Override
              public /*KV<K, ValueArrayT>*/ byte[] call(/*KV<K, InputT>*/ byte[] input) {
                WindowedValue<KV<K, ValueInputT>> wkvi =
                    CoderHelpers.fromByteArray(input, wkviCoder);
                ValueArrayT va = keyed.createAccumulator(wkvi.getValue().getKey());
                va = keyed.addInput(wkvi.getValue().getKey(), va, wkvi.getValue().getValue());
                WindowedValue<KV<K, ValueArrayT>> wkva =
            ...
            new Function2</*KV<K, ValueArrayT>*/ byte[],
                /*KV<K, InputT>*/ byte[],
                /*KV<K, ValueArrayT>*/ byte[]>() {
              @Override
              public /*KV<K, ValueArrayT>*/ byte[] call(/*KV<K, ValueArrayT>*/ byte[] acc,
            ...
            new Function2</*KV<K, ValueArrayT>*/ byte[],
                ...
                /*KV<K, ValueArrayT>*/ byte[]>() {
              @Override
              public /*KV<K, ValueArrayT>*/ byte[] call(/*KV<K, ValueArrayT>*/ byte[] acc1,
                  /*KV<K, ValueArrayT>*/ byte[] acc2) {
                WindowedValue<KV<K, ValueArrayT>> wkva1 =
                    CoderHelpers.fromByteArray(acc1, wkvaCoder);
                WindowedValue<KV<K, ValueArrayT>> wkva2 =
                    CoderHelpers.fromByteArray(acc2, wkvaCoder);
                ValueArrayT va = keyed.mergeAccumulators(wkva1.getValue().getKey(),
                    // don't use Guava's ImmutableList.of as values may be null
        ...
        JavaPairRDD<WindowedValue<K>, WindowedValue<ValueOutputT>> extracted = accumulatedBytes
            .mapToPair(CoderHelpers.fromByteFunction(wkCoder, wkvaCoder))
            .mapValues(
                new Function<WindowedValue<KV<K, ValueArrayT>>, WindowedValue<ValueOutputT>>() {
                  @Override
                  public WindowedValue<ValueOutputT> call(WindowedValue<KV<K, ValueArrayT>> acc) {
        ...
        context.setOutputRDD(transform,
            fromPair(extracted)
            .map(new Function<KV<WindowedValue<K>, WindowedValue<ValueOutputT>>,
                WindowedValue<KV<K, ValueOutputT>>>() {
              @Override
              public WindowedValue<KV<K, ValueOutputT>> call(KV<WindowedValue<K>,
                  WindowedValue<ValueOutputT>> kwvo)
                  throws Exception {
                WindowedValue<ValueOutputT> wvo = kwvo.getValue();
                KV<K, ValueOutputT> kvo = KV.of(kwvo.getKey().getValue(), wvo.getValue());
  ...
  private static final class KVFunction<K, ValueInputT, ValueOutputT>
      implements Function<WindowedValue<KV<K, Iterable<ValueInputT>>>,
      WindowedValue<KV<K, ValueOutputT>>> {
    private final Combine.KeyedCombineFn<K, ValueInputT, ?, ValueOutputT> keyed;

     KVFunction(Combine.KeyedCombineFn<K, ValueInputT, ?, ValueOutputT> keyed) {
      this.keyed = keyed;
    }

    @Override
    public WindowedValue<KV<K, ValueOutputT>> call(WindowedValue<KV<K,
        Iterable<ValueInputT>>> windowedKv)
        throws Exception {
      KV<K, Iterable<ValueInputT>> kv = windowedKv.getValue();
