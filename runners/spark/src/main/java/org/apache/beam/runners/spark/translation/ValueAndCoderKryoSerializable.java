 ...
 *
 * The deserialized representation stores a Coder and the value. To serialize the value, we write a length-prefixed
 * encoding of value, but do NOT write the Coder used.
 *
 * The serialized representation just reads a byte array - the value is not deserialized fully. In order to get at
 * the deserialized value, the caller must pass the Coder used to create this instance via get(Coder). This reverts the
 * representation back to the deserialized representation.
 *
...
public class ValueAndCoderKryoSerializable<T> implements KryoSerializable, Externalizable {
    ...
    public T get(Coder<T> coder) throws IOException {
        if (!(coderOrBytes instanceof Coder)) {
            value = coder.decode(new ByteArrayInputStream((byte[]) this.coderOrBytes), Coder.Context.OUTER);
