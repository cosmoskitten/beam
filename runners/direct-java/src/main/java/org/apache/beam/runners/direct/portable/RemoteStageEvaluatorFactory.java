...
class RemoteStageEvaluatorFactory implements TransformEvaluatorFactory {
  ...
  RemoteStageEvaluatorFactory(BundleFactory bundleFactory,
  ...
  private class RemoteStageEvaluator<T> implements TransformEvaluator<T> {
    ...
    private RemoteStageEvaluator(
        PTransformNode transform, EvaluationContext evaluationContext) throws Exception {
      ...
      StateRequestHandler handler = getStateRequestHandler(
          stage,
          stageFactory.getProcessBundleDescriptor(),
          evaluationContext);
    ...
    private StateRequestHandler getStateRequestHandler(
      ...
      EnumMap<TypeCase, StateRequestHandler> handlerMap =
