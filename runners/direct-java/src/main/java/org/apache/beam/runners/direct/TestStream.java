...
public class TestStream<T> extends PTransform<PBegin, PCollection<T>> {
  ...
  private void setup(Pipeline p) {
    ...
    DirectRunner.InternalOptions directOptions = options.as(DirectRunner.InternalOptions.class);
    ...
    Map<Class<? extends PTransform>, OptionsSupplier<TransformEvaluatorFactory>>
        additionalTransformEvaluators = directOptions.getAdditionalTransformEvaluators();
    if (!additionalTransformEvaluators.containsKey(TestStream.class)) {
      directOptions.setAdditionalTransformEvaluators(
          ImmutableMap
              .<Class<? extends PTransform>, OptionsSupplier<TransformEvaluatorFactory>>builder()
              .putAll(additionalTransformEvaluators)
              .put(TestStream.class, new FactorySupplier())
              .build());
    }
  }

  private static class FactorySupplier implements OptionsSupplier<TransformEvaluatorFactory> {
    @Override
    public TransformEvaluatorFactory get() {
