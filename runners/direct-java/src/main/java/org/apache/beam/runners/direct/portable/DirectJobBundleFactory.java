        ...
        String bundleOutputPCollection =
            Iterables.getOnlyElement(
                descriptor
                    ...
                    .getInputsMap()
                    .values());
        FnDataReceiver<WindowedValue<?>> outputPCollection =
            outputReceiverFactory.create(bundleOutputPCollection);
        outputReceivers.put(
            targetCoders.getKey(),
            RemoteOutputReceiver.of(targetCoders.getValue(), outputPCollection));
      }
      ActiveBundle<T> bundle = processor.newBundle(outputReceivers, stateRequestHandler);
      MoreFutures.get(processor.getRegistrationFuture());
      return bundle;
    }
