...
final class StatefulParDoEvaluatorFactory<K, InputT, OutputT> implements TransformEvaluatorFactory {
  ...
  private TransformEvaluator<InputT> createEvaluator(
      AppliedPTransform<PCollection<? extends KeyedWorkItem<K, InputT>>, PCollectionTuple,
          StatefulParDo<K, InputT, OutputT>>
          application,
      CommittedBundle<KeyedWorkItem<K, InputT>> inputBundle)
      throws Exception {
  ...
  private class CleanupSchedulingLoader
    ...
    public Runnable load(
      ...
      PCollection<?> pc =
          transformOutputWindow.getTransform().getOutput().get(
              transformOutputWindow.getTransform().getTransform().
                  getUnderlyingParDo().getMainOutputTag());
  ...
  abstract static class AppliedPTransformOutputKeyAndWindow<K, InputT, OutputT> {
    abstract AppliedPTransform<
            PCollection<? extends KeyedWorkItem<K, InputT>>, PCollectionTuple, StatefulParDo<K,
        InputT, OutputT>>
        getTransform();
  ...
  private static class StatefulParDoEvaluator<K, InputT> implements
      TransformEvaluator<KeyedWorkItem<K, InputT>> {
    ...
    @Override
    public void processElement(WindowedValue<KeyedWorkItem<K, InputT>> keyedWorkItem) throws
        Exception {
      for (WindowedValue<InputT> elem : keyedWorkItem.getValue().elementsIterable()) {
        delegateEvaluator.processElement(
            elem.withValue(KV.of(keyedWorkItem.getValue().key(), elem.getValue())));
      }
    }
