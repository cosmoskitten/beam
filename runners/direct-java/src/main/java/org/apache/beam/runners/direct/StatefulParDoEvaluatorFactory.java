...
final class StatefulParDoEvaluatorFactory<K, InputT, OutputT> implements TransformEvaluatorFactory {
  ...
  private TransformEvaluator<InputT> createEvaluator(
      AppliedPTransform<
              PCollection<? extends KeyedWorkItem<K, InputT>>, PCollectionTuple,
  ...
  private static class StatefulParDoEvaluator<K, InputT>
      implements TransformEvaluator<KeyedWorkItem<K, InputT>> {
    ...
    @Override
    public void processElement(WindowedValue<KeyedWorkItem<K, InputT>> keyedWorkItem)
        throws Exception {
