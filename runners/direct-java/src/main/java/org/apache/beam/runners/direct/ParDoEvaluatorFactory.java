...
final class ParDoEvaluatorFactory<
  ...
  private final LoadingCache<AppliedPTransformOutputKeyAndWindow, Boolean> cleanupRegistry;
  ...
  ParDoEvaluatorFactory(
    ...
    this.cleanupRegistry =
        CacheBuilder.newBuilder().build(new CleanupSchedulingLoader(evaluationContext));
    fnClones =
  ...
  private TransformEvaluator<InputT> createEvaluator(
    ...
    // If the DoFn is stateful, schedule state clearing idempotently
  ...
  private class CleanupSchedulingLoader
      extends CacheLoader<AppliedPTransformOutputKeyAndWindow, Boolean> {
    ...
    @Override
    public Boolean load(final AppliedPTransformOutputKeyAndWindow transformOutputWindow) {
