...
import org.apache.beam.runners.dataflow.worker.BeamFnMapTaskExecutorFactory;
...
public class RegisterNodeFunction implements Function<MutableNetwork<Node, Edge>, Node> {
  ...
  public Node apply(MutableNetwork<Node, Edge> input) {
    ...
    LOG.error("migryz RegisterNodeFunction.apply stack:\n{}", getStackTrace());
    ...
    ImmutableMap.Builder<String, Iterable<PCollectionView<?>>> ptransformIdToPCollectionViews =
        ImmutableMap.builder();
    ImmutableMap.Builder<String, NameContext> pcollectionIdToNameContexts =
        ImmutableMap.builder();


    //todomigryz extract contents of this loop to separate descriptive method
    ...
    final Iterable<InstructionOutputNode> instructionOutputNodes = Iterables
        .filter(input.nodes(), InstructionOutputNode.class);
    for (InstructionOutputNode node : instructionOutputNodes) {
      ...
      pcollectionIdToNameContexts.put(pcollectionId,
          NameContext
              .create(null,
