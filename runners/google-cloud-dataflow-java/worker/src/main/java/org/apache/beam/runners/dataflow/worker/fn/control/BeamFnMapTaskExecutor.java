...
public class BeamFnMapTaskExecutor extends DataflowMapTaskExecutor {
  ...
  private static String counterUpdateListToString(Iterable<CounterUpdate> src) {
  ...
  public Iterable<CounterUpdate> extractMetricUpdates() {
    ...
    LOG.error("migryz extractMetricsUpdates\n" + counterUpdateListToString(result));
    ...
    LOG.error("migryz extractMetricUpdates falling back to metrics\n" + counterUpdateListToString(
        deprecatedMetrics));
  ...
  /*package*/ static class SingularProcessBundleProgressTracker implements ProgressTracker {
    ...
    void updateProgress() {
      try {
        ...
        // Requires Count metrics to be implemented to test.
    ...
    private void updateMetrics(List<MonitoringInfo> monitoringInfos) {
      LOG.error("migryz updateMetrics start");
