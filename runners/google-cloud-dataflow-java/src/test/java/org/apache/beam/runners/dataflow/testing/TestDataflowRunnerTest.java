...
public class TestDataflowRunnerTest {
   ...
   *
   * <p>Currently, this failure is indistinguishable from a non-{@link PAssert} failure, because it is detected
   * only by failure job messages. With fuller metric support, this can detect a PAssert failure via metrics
   * and raise an {@link AssertionError} in just that case.
   */
  @Test
  ...
  /**
   * Tests that a tentative {@code true} from metrics indicates that every {@link PAssert} has succeeded.
   */
  @Test
   ...
   * that the test run throws an {@link AssertionError}.
   *
   * <p>This is a known limitation/bug of the runner that it does not distinguish the two modes of failure.
   */
  @Test
