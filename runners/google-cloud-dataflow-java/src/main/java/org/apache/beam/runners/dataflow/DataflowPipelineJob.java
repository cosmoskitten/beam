...
public class DataflowPipelineJob implements PipelineResult {
  ...
  State waitUntilFinish(
    ...
    FlexibleBackoff backoff = FlexibleBackoff.of()
        .withMaxAttempts(MESSAGES_POLLING_ATTEMPTS)
    ...
    do {
      ...
      if (!hasError) {
        // Reset the backoff, and update the new cumulative sleep time to be the remaining
  ...
  private Job getJobWithRetries(int attempts, Sleeper sleeper) throws IOException {
    BackOff backoff =
        FlexibleBackoff.of().withMaxAttempts(attempts).withInitialBackoff(STATUS_POLLING_INTERVAL);
