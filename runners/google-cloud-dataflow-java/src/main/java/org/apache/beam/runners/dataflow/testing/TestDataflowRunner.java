...
public class TestDataflowRunner extends PipelineRunner<DataflowPipelineJob> {
  ...
  Optional<Boolean> checkForPAssertSuccess(DataflowPipelineJob job) throws IOException {
    ...
    if (failures > 0) {
      ...
      LOG.info("Success result for Dataflow job {}."
          + " Found {} success, {} failures out of {} expected assertions.", job.getJobId(), successes, failures,
          expectedNumberOfAssertions);
      return Optional.of(true);
    }

    LOG.info("Inconclusive results for Dataflow job {}."
        + " Found {} success, {} failures out of {} expected assertions.",
        job.getJobId(), successes, failures, expectedNumberOfAssertions);
    return Optional.absent();
  }
