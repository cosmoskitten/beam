...
import org.apache.beam.runners.fnexecution.control.DefaultExecutableStageContext.JobFactoryState;
...
public class DefaultExecutableStageContextTest {
  ...
  public void testMultiInstanceFactory() {
    ...
    JobFactoryState jobFactoryState = new JobFactoryState(2, (x) -> true);

    WrappedContext f1 = (WrappedContext) jobFactoryState.getFactory().get(jobInfo);
    WrappedContext f2 = (WrappedContext) jobFactoryState.getFactory().get(jobInfo);
    WrappedContext f3 = (WrappedContext) jobFactoryState.getFactory().get(jobInfo);
  ...
  public void testDefault() {
    ...
    JobFactoryState jobFactoryState = new JobFactoryState(0, (x) -> true);
    ...
    for (int i = 1; i < expectedParallelism; i++) {
      Assert.assertNotEquals(
          ...
          f1.context,
          ((WrappedContext) jobFactoryState.getFactory().get(jobInfo)).context);
    }
