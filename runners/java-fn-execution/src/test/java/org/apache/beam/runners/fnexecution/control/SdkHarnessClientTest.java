  ...
  public void testNewBundleAndProcessElements() throws Exception {
    ...
    BundleProcessor<String> processor =
        client.getProcessor(
            ...
            RemoteInputDestination.of(
                FullWindowedValueCoder.of(StringUtf8Coder.of(), Coder.INSTANCE), inputTarget));
  ...
  private BeamFnApi.ProcessBundleDescriptor getProcessBundleDescriptor(
    ...
    RemoteGrpcPortRead readNode =
        RemoteGrpcPortRead.readFromPort(
            port, Iterables.getOnlyElement(targetProcessor.getInputsMap().values()));
    RemoteGrpcPortWrite writeNode =
        RemoteGrpcPortWrite.writeToPort(
            Iterables.getOnlyElement(targetProcessor.getOutputsMap().values()), port);
    // TODO: GrpcPortRead to GrpcPortWrite over the Fn API
    // TODO: Fiddle with coders on all pcollections
