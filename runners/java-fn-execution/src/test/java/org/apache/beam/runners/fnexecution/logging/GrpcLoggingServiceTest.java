...
import org.apache.beam.runners.fnexecution.CountDown;
...
public class GrpcLoggingServiceTest {
  ...
  public void testMultipleClientsSuccessfullyProcessed() throws Exception {
    ...
    GrpcLoggingService service =
        GrpcLoggingService.forWriter(new ConcurrentCollectionLogWriter(logs));
  ...
  private static class ConcurrentCollectionLogWriter implements LogWriter {
    private final Collection<BeamFnApi.LogEntry> entries;

    private ConcurrentCollectionLogWriter(Collection<LogEntry> entries) {
