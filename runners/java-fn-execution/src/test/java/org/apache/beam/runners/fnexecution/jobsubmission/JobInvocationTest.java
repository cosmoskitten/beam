...
public class JobInvocationTest {
  ...
  public void testStateAfterCancellationWithPipelineResult() throws Exception {
    ...
    runner.setResult(pipelineResult);
    awaitChangingJobState(jobInvocation, JobApi.JobState.Enum.RUNNING, JobApi.JobState.Enum.FAILED);
  ...
  public void testNoCancellationWhenDone() throws Exception {
    ...
    assertThat(pipelineResult.cancelLatch.getCount(), is(0));
  }

  private static void awaitChangingJobState(
      JobInvocation jobInvocation, JobApi.JobState.Enum startState, JobApi.JobState.Enum endState)
      throws Exception {
    JobApi.JobState.Enum currentState = jobInvocation.getState();
    Preconditions.checkState(currentState == startState);
    while (currentState == startState) {
      Thread.sleep(50);
      currentState = jobInvocation.getState();
    }
    assertThat(currentState, is(endState));
  }
