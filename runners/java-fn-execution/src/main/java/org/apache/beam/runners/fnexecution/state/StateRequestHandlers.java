...
public class StateRequestHandlers {
    ...
    public CompletionStage<StateResponse.Builder> handle(
      ...
      try {
        ...
        switch (request.getRequestCase()) {
          case GET:
            return handleGetRequest(request, key, window, referenceSpec.valueCoder(), handler::get);
          case APPEND:
          case CLEAR:
          default:
  ...
  static class ByteStringStateRequestHandlerToBagUserStateHandlerFactoryAdapter
    ...
    public CompletionStage<StateResponse.Builder> handle(
      ...
      try {
        ...
        switch (request.getRequestCase()) {
          case GET:
            return handleGetRequest(request, key, window, ByteStringCoder.of(), handler::get);
          case APPEND:
    ...
    private <W extends BoundedWindow> CompletionStage<StateResponse.Builder>
    handleAppendRequest(
  ...
  private interface KeyedGetHandler<K, V, W extends BoundedWindow> {
    Iterable<V> get(K key, W window);
  }

  private static <K, V, W extends BoundedWindow> CompletionStage<StateResponse.Builder> handleGetRequest(
      StateRequest request,
      K key,
      W window,
      Coder<V> valueCoder,
      KeyedGetHandler<K, V, W> handler) throws Exception {
