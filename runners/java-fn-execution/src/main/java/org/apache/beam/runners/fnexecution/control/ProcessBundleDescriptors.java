...
import org.apache.beam.runners.core.construction.graph.KeyedStateReference;
...
public class ProcessBundleDescriptors {
  ...
  private static ExecutableProcessBundleDescriptor fromExecutableStageInternal(
    ...
    Map<String, Map<String, BagUserStateSpec>> bagUserStateSpecs =
        forBagUserStates(stage, components);
  ...
  private static Map<String, Map<String, BagUserStateSpec>> forBagUserStates(
    ...
    for (KeyedStateReference keyedStateReference : stage.getKeyedStates()) {
      FullWindowedValueCoder<KV<?, ?>> coder =
          (FullWindowedValueCoder) WireCoders.instantiateRunnerWireCoder(
              keyedStateReference.collection(), components);
      idsToSpec.put(
          keyedStateReference.transform().getId(),
          keyedStateReference.localName(),
              BagUserStateSpec.of(
              keyedStateReference.transform().getId(),
              keyedStateReference.localName(),
