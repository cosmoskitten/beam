...
function get_leader() {
  ...
  while read line; do
    ...
    i=$((i+1))
  done <<< $(gcloud compute ssh yarn@$MASTER_NAME --command="yarn application -list" --zone=$GCLOUD_ZONE --quiet | grep "$CLUSTER_NAME")
...
function start_tunnel() {
  local job_server_config=`gcloud compute ssh --quiet --zone=$GCLOUD_ZONE yarn@$MASTER_NAME --command="curl -s \"http://$YARN_APPLICATION_MASTER/jobmanager/config\"" --zone=$GCLOUD_ZONE --quiet`
  ...
  local tunnel_command="gcloud compute ssh yarn@${MASTER_NAME} -- -L ${FLINK_LOCAL_PORT}:${YARN_APPLICATION_MASTER} -L ${jobmanager_rpc_port}:${yarn_application_master_host}:${jobmanager_rpc_port} -D 1080 ${detached_mode_params} --zone=$GCLOUD_ZONE --quiet"
