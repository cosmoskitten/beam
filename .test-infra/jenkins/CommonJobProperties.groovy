...
class CommonJobProperties {
  ...
  static def buildPerformanceTest(def context, def argMap) {
    ...
    context.steps {
        ...
        shell("-c virtualenv ${beam_env}")
        shell("-c virtualenv ${perfkit_env}")

        // update setuptools and pip
        shell("-c ${beam_env}/bin/pip install --upgrade grpcio-tools==1.3.5")
        ...
        shell("-c cd ${beam_root}/sdks/python && ${beam_env}/bin/pip install -e .[gcp,test]")

        // Build PythonSDK tar ball.
        shell("-c cd ${beam_root}/sdks/python && ${beam_env}/bin/python setup.py sdist --dist-dir=target")

        // Install Perfkit benchmark requirements.
        shell("-c cd ${perfkit_root} && ${perfkit_env}/bin/pip install -r requirements.txt")

        // Launch performance test.
        shell("-c cd ${perfkit_root} && ${perfkit_env}/bin/python pkb.py ${pkbArgs}")
    }
  }
