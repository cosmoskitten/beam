...
def runTest = { int numberOfWorkers, int clusterSize, scope, sdkName, parametrizedTestConfigurations, testName, testType ->
    if(numberOfWorkers != clusterSize)
...
def getNumbersOfWorkersSortedDesc = { it.collect { item -> item.jobProperties?.parallelism }.unique().sort().reverse() }
...
def loadTest = { scope, triggeringContext ->
  ...
  def numbersOfWorkers = getNumbersOfWorkersSortedDesc(parametrizedTestConfigurations)
  def clusterSize = numbersOfWorkers[0]
  ...
  infra.setupFlinkCluster(scope, jenkinsJobName, flinkDownloadUrl, pythonHarnessImageTag, jobServerImageTag, clusterSize)

  numbersOfWorkers.each { runTest(it, clusterSize,  scope, sdkName,  parametrizedTestConfigurations, "GBK", "batch") }
