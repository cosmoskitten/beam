  ...
  try:
    ...
    if not is_currently_used_bool:
      ...
      if currently_used_version_in_db is not None:
        ...
        bigquery_client.insert_dep_to_table2(dep_name, currently_used_version_in_db, currently_used_release_date_in_db, is_current_using=False)
      if curr_release_date is not None:
        print 1.2
        bigquery_client.delete_dep_from_table(dep_name, curr_ver)
      bigquery_client.insert_dep_to_table2(dep_name, curr_ver, date_today, is_current_using=True)
    # sync to the bigquery table on the dependency status of the latest version.
    if latest_release_date is None:
      bigquery_client.insert_dep_to_table2(dep_name, latest_ver, date_today, is_current_using=False)
