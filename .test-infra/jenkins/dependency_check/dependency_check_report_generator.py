...
def query_dependency_release_dates(bigquery_client, dep_name, curr_ver, latest_ver):
  ...
  """
  try:
    curr_release_date, is_currently_used_bool = bigquery_client.query_dep_info_by_version(dep_name, curr_ver)
    print "{0},  {1}".format(curr_release_date, is_currently_used_bool)
    ...
    if not is_currently_used_bool:
      ...
      if currently_used_version_in_db is not None:
        ...
        bigquery_client.insert_dep_to_table(dep_name, currently_used_version_in_db, currently_used_release_date_in_db, is_current_using=False)
      if curr_release_date is not None:
        bigquery_client.delete_dep_from_table(dep_name, curr_ver)
      bigquery_client.insert_dep_to_table(dep_name, curr_ver, date_today, is_current_using=True)
