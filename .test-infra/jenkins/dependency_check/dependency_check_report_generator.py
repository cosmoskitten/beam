...
def query_dependency_release_dates(bigquery_client, dep_name, curr_ver_in_beam, latest_ver):
  ...
  try:
    ...
    if not is_currently_used_bool:
      currently_used_release_date_in_db, currently_used_release_date_in_db = bigquery_client.query_currently_used_dep_info_in_db(dep_name)
      if currently_used_release_date_in_db is not None:
        bigquery_client.delete_dep_from_table(dep_name, currently_used_release_date_in_db)
        bigquery_client.insert_dep_to_table(dep_name, currently_used_release_date_in_db, currently_used_release_date_in_db, is_current_using=False)
      if curr_release_date is not None:
        bigquery_client.delete_dep_from_table(dep_name, curr_ver_in_beam)
      bigquery_client.insert_dep_to_table(dep_name, curr_ver_in_beam, date_today, is_current_using=True)
    # sync to the bigquery table on the dependency status of the latest version.
    if latest_release_date is None:
      bigquery_client.insert_dep_to_table(dep_name, latest_ver, date_today, is_current_using=False)
