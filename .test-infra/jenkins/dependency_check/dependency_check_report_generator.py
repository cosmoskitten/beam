...
def query_dependency_release_dates(bigquery_client, dep_name, curr_ver, latest_ver):
  ...
  """
  try:
    curr_release_date, is_currently_used_bool = bigquery_client.query_dep_release_date(dep_name, curr_ver)
    latest_release_date = bigquery_client.query_dep_release_date(dep_name,latest_ver)
    date_today = datetime.today().date()

    # updating status of the currently used version.
    if not is_currently_used_bool:
      print 1
      currently_used_version = bigquery_client.query_currently_used_version(dep_name)
      if currently_used_version is not None:
        print 1.2
        bigquery_client.update_dep_to_table(dep_name, currently_used_version, is_current_using=False)
      if curr_release_date is None:
        print 1.3
        bigquery_client.add_dep_to_table(dep_name, curr_ver, date_today, is_current_using=True)
      else:
        print 1.4
        bigquery_client.update_dep_to_table(dep_name, curr_ver, is_current_using=True)
    if latest_release_date is None:
      bigquery_client.add_dep_to_table(dep_name, latest_ver, date_today, is_current_using=False)
