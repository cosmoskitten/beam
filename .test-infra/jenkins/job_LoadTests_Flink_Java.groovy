...
def loadTestJob = { scope, triggeringContext ->
  ...
  // Has to be done in a step, otherwise BUILD_TAG will be set from "seed_job"
  scope.steps {
    String clusterName = "flink-${BUILD_TAG}".toLowerCase().replace("_", "-")
    shell("export CLUSTER_NAME=${clusterName}")
  }
