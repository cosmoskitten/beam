...
def loadTestJob = { scope, jobName, triggeringContext ->
  ...
  def clusterName = "${jobName.toLowerCase().replace("_", "-")}-$BUILD_ID"
  infrastructure.setupFlinkCluster(scope, clusterName, 5)
  ...
  infrastructure.teardownDataproc(scope, clusterName)
}
