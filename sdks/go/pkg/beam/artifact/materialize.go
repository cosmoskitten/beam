...
// present.
func Materialize(ctx context.Context, endpoint string, retrievalToken string, dest string) ([]*pb.ArtifactMetadata, error) {
	...
	m, err := client.GetManifest(ctx, &pb.GetManifestRequest{RetrievalToken: retrievalToken})
	if err != nil {
		return nil, fmt.Errorf("failed to get manifest: %v", err)
	}
	md := m.GetManifest().GetArtifact()
	return md, MultiRetrieve(ctx, client, 10, md, retrievalToken, dest)
}
...
func MultiRetrieve(ctx context.Context, client pb.ArtifactRetrievalServiceClient, cpus int, list []*pb.ArtifactMetadata, retrievalToken string, dest string) error {
	if len(list) == 0 {
		return nil
	}
	if cpus < 1 {
	...
	for i := 0; i < cpus; i++ {
		...
		go func() {
			...
			for a := range q {
				...
				var failures []string
				for {
					err := Retrieve(ctx, client, a, retrievalToken, dest)
...
// failure.
func Retrieve(ctx context.Context, client pb.ArtifactRetrievalServiceClient, a *pb.ArtifactMetadata, retrievalToken string, dest string) error {
	...
	return retrieve(ctx, client, a, retrievalToken, filename)
}
...
func retrieve(ctx context.Context, client pb.ArtifactRetrievalServiceClient, a *pb.ArtifactMetadata, retrievalToken string, filename string) error {
	stream, err := client.GetArtifact(ctx, &pb.GetArtifactRequest{Name: a.Name, RetrievalToken: retrievalToken})
	if err != nil {
		return err
	}
