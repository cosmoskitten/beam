...
func TestDirtyRetrieve(t *testing.T) {
	...
	ugly := filepath.Join(dst, "ugly")

	makeTempFile(t, good, 500) // correct content. Do nothing.
	makeTempFile(t, bad, 367)  // invalid content. Delete and retrieve.
	makeTempFile(t, ugly, 500) // invalid permissions. Delete and retrieve.
	if err := os.Chmod(ugly, 0731); err != nil {
		t.Errorf("failed to chmod ugly file: %v", err)
	}

	rcl := pb.NewArtifactRetrievalServiceClient(cc)
	if err := MultiRetrieve(ctx, rcl, 3, list, dst); err != nil {
		t.Fatalf("failed to get retrieve: %v", err)
	}

	verifyMD5(t, good, list[0].Md5)
	verifyMD5(t, bad, list[1].Md5)
	verifyMD5(t, ugly, list[2].Md5)
	if stat, _ := os.Stat(ugly); stat.Mode() != 0644 {
		t.Errorf("ugly file has bad permissions: %v, want 0644", stat.Mode())
	}
}
