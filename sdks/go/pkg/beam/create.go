...
func addContext(err error, s Scope) error {
...
func TryCreate(s Scope, values ...interface{}) (PCollection, error) {
	if len(values) == 0 {
		return PCollection{}, addContext(errors.New("create has no values"), s)
	}
	...
	for i, value := range values {
		...
		if err := enc.Encode(value, &buf); err != nil {
			return PCollection{}, addContext(errors.Wrapf(err, "marshalling of %v failed", value), s)
		}
	...
	if err != nil || len(ret) != 1 {
		panic(addContext(errors.WithContext(err, "internal error"), s))
	}
	return ret[0], nil
}
