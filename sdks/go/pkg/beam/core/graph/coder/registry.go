...
func RegisterCoder(t reflect.Type, enc, dec interface{}) {
	...
	coderRegistry[key] = func() *CustomCoder {
		cc, err := NewCustomCoder(t.String(), t, enc, dec)
		if err != nil {
			// An error on look up shouldn't happen after the validation.
			panic(fmt.Sprintf("Creating CustomCoder failed for type %t: %v", t, err))
		}
		return cc
	}
}
