...
func clearRegistry() {
	coderRegistry = make(map[uintptr]func() *CustomCoder)
...
func TestRegisterCoder(t *testing.T) {
	tests := []struct {
		...
		name: "satisfyingInterface",
		typ:  msType, enc: cEnc, dec: cDec,
	}, {
		name: "ptrSatisfyingInterfaceDirect",
		typ:  msPtrType, enc: bEnc, dec: bDec,
	}, {
		name: "ptrSatisfyingInterfacePromoted",
		typ:  msPtrType, enc: cEnc, dec: cDec,
	}, {
		name: "swapped",
...
func TestLookupCustomCoder(t *testing.T) {
	newCC := func(t *testing.T, et reflect.Type, enc, dec interface{}) *CustomCoder {
		t.Helper()
		cc, err := NewCustomCoder(et.String(), et, enc, dec)
		if err != nil {
	...
	miCC := newCC(t, miType, miEnc, miDec)
	msCC := newCC(t, msType, msEnc, msDec)
	msPtrCC := newCC(t, msPtrType, msPtrEnc, msPtrDec)
	aCC := newCC(t, aType, aEnc, aDec)
	bCC := newCC(t, bType, bEnc, bDec)
	cCC := newCC(t, cType, cEnc, cDec)
