...
func TestValidEncoderForms(t *testing.T) {
	type EncType struct{}
	eT := reflect.TypeOf((*EncType)(nil)).Elem()

	tests := []struct {
		enc interface{}
	}{
		{enc: func(EncType) []byte { return nil }},
		{enc: func(EncType) ([]byte, error) { return nil, nil }},
		{enc: func(reflect.Type, EncType) []byte { return nil }},
		{enc: func(reflect.Type, EncType) ([]byte, error) { return nil, nil }},
	}
	for _, test := range tests {
		test := test
		t.Run(fmt.Sprintf("%T", test.enc), func(t *testing.T) {
			if err := validateEncoder(eT, test.enc); err != nil {
				t.Fatal(err)
			}
		})
	}
...
func TestValidDecoderForms(t *testing.T) {
	type DecType struct{}
	dT := reflect.TypeOf((*DecType)(nil)).Elem()

	tests := []struct {
		dec interface{}
	}{
		{dec: func([]byte) DecType { return DecType{} }},
		{dec: func([]byte) (DecType, error) { return DecType{}, nil }},
		{dec: func(reflect.Type, []byte) DecType { return DecType{} }},
		{dec: func(reflect.Type, []byte) (DecType, error) { return DecType{}, nil }},
	}
	for _, test := range tests {
		test := test
		t.Run(fmt.Sprintf("%T", test.dec), func(t *testing.T) {
			if err := validateDecoder(dT, test.dec); err != nil {
				t.Fatal(err)
			}
		})
	}
