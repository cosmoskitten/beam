...
func TestParDo(t *testing.T) {
	...
	p, err := proto.Marshal(&pb.DockerPayload{ContainerImage: "foo"})
	if err != nil {
		t.Fatal(err)
	}
	payload, err := graphx.Marshal(edges,
		&graphx.Options{Environment: pb.Environment{Url: "foo", Urn: "beam:env:docker:v1", Payload: p}})
	if err != nil {
		t.Fatal(err)
	}

	if len(payload.GetComponents().GetTransforms()) != 1 {
		t.Errorf("bad ParDo translation: %v", proto.MarshalTextString(payload))
	}
}
