...
func EncodeMultiEdge(edge *graph.MultiEdge) (*v1.MultiEdge, error) {
	...
	if edge.DoFn != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to encode multi edge %v, bad userfn: %v", *edge, err)
		}
		ret.Fn = ref
	}
	if edge.CombineFn != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to encode multi edge %v, bad combinefn: %v", *edge, err)
		}
		ret.Fn = ref
	}
	...
	for _, in := range edge.Input {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to encode multi edge %v, bad input type: %v", *edge, err)
		}
	...
	for _, out := range edge.Output {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to encode multi edge %v, bad output type: %v", *edge, err)
		}
...
func DecodeMultiEdge(edge *v1.MultiEdge) (graph.Opcode, *graph.Fn, *window.Fn, []*graph.Inbound, []*graph.Outbound, error) {
	...
	if edge.Fn != nil {
		...
		if err != nil {
			return "", nil, nil, nil, nil, fmt.Errorf("failed to decode multi edge %v, bad userfn: %v", *edge, err)
		}
	}
	if edge.WindowFn != nil {
	...
	for _, in := range edge.Inbound {
		...
		if err != nil {
			return "", nil, nil, nil, nil, fmt.Errorf("failed to decode multi edge %v, bad input kind: %v", *edge, err)
		}
		t, err := decodeFullType(in.Type)
		if err != nil {
			return "", nil, nil, nil, nil, fmt.Errorf("failed to decode multi edge %v, bad input type: %v", *edge, err)
		}
	...
	for _, out := range edge.Outbound {
		...
		if err != nil {
			return "", nil, nil, nil, nil, fmt.Errorf("failed to decode multi edge %v, bad output type: %v", *edge, err)
		}
...
func encodeCustomCoder(c *coder.CustomCoder) (*v1.CustomCoder, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to encode custom coder %v, bad underlying type: %v", *c, err)
	}
...
func decodeCustomCoder(c *v1.CustomCoder) (*coder.CustomCoder, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to decode custom coder proto %v, bad underlying type: %v", *c, err)
	}
	enc, err := decodeUserFn(c.Enc)
	if err != nil {
		return nil, fmt.Errorf("failed to decode custom coder proto %v, bad encoding function: %v", *c, err)
	}
	dec, err := decodeUserFn(c.Dec)
	if err != nil {
		return nil, fmt.Errorf("failed to decode custom coder proto %v, bad decoding function: %v", *c, err)
	}

	ret, err := coder.NewCustomCoder(c.Name, t, enc, dec)
	if err != nil {
		return nil, fmt.Errorf("failed to decode custom coder proto %v: %v", *c, err)
	}
	return ret, nil
}
...
func encodeFn(u *graph.Fn) (*v1.Fn, error) {
	...
	case u.DynFn != nil:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to encode function %v, bad function type: %v", *u, err)
		}
	...
	case u.Fn != nil:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to encode function %v, bad userfn: %v", *u, err)
		}
	...
	case u.Recv != nil:
		...
		if !ok {
			return nil, fmt.Errorf("failed to encode function %v, bad receiver %v", *u, u.Recv)
		}
		if _, ok := runtime.LookupType(k); !ok {
			return nil, fmt.Errorf("failed to encode function %v, receiver type %v must be registered", *u, t)
		}
		typ, err := encodeType(t)
		if err != nil {
			panic(fmt.Sprintf("failed to encode function %v, bad receiver type: %v", *u, err))
		}
	...
	default:
		panic(fmt.Errorf("failed to encode function %v, empty fn", *u))
	}
}
...
func decodeFn(u *v1.Fn) (*graph.Fn, error) {
	if u.Dynfn != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to decode function proto %v, bad symbol %v: %v", *u, u.Dynfn.Gen, err)
		}

		t, err := decodeType(u.Dynfn.Type)
		if err != nil {
			return nil, fmt.Errorf("failed to decode function proto %v, bad type: %v", *u, err)
		}
	...
	if u.Fn != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to decode function proto %v, bad userfn: %v", *u, err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to decode function proto %v, bad struct encoding: %v", *u, err)
	}
...
func encodeUserFn(u *funcx.Fn) (*v1.UserFn, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to encode user fn %v, bad function type: %v", *u, err)
	}
...
func decodeType(t *v1.Type) (reflect.Type, error) {
	if t == nil {
		return nil, fmt.Errorf("failed to decode type proto %v, empty type", *t)
	}
	...
	case v1.Type_SLICE:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to decode type proto %v, bad element: %v", *t, err)
		}
	...
	case v1.Type_STRUCT:
		...
		for _, f := range t.Fields {
			...
			if err != nil {
				return nil, fmt.Errorf("failed to decode type proto %v, bad field type: %v", *t, err)
			}
	...
	case v1.Type_FUNC:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to decode type proto %v, bad parameter type: %v", *t, err)
		}
		out, err := decodeTypes(t.GetReturnTypes())
		if err != nil {
			return nil, fmt.Errorf("failed to decode type proto %v, bad return type: %v", *t, err)
		}
	...
	case v1.Type_CHAN:
		...
		if err != nil {
			return nil, fmt.Errorf("failed to decode type proto %v, bad channel direction: %v", *t, err)
		}
	...
	case v1.Type_EXTERNAL:
		...
		if !ok {
			return nil, fmt.Errorf("failed to decode type proto %v, external key not found %v", *t, t.ExternalKey)
		}
		return ret, nil

	default:
		return nil, fmt.Errorf("failed to decode type proto %v, unexpected type kind %v", *t, t.Kind)
	}
}
