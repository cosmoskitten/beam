...
//
// Containers (or metric containers) are defined for each Beam model metric
...
// ptransform code. They don't retain their containers, since they don't have
...
// Metric containers are segregated by name, ptransform, and bundle,
...
var (
	...
	// We store the user path access to the containers in metric type segregated
	// sync.Maps. Using sync.Maps lets metrics with disjoint keys have concurrent
	// access to the containers, and using separate sync.Map per metric type
...
// counter is a metric container for counter values.
type counter struct {
	value int64
	mu    sync.Mutex
}
...
// distribution is a metric container for distribution values.
type distribution struct {
...
func storeMetric(key key, m userMetric) {
	...
	if _, ok := metricStorage[key.bundle]; !ok {
		metricStorage[key.bundle] = make(map[string]map[name]userMetric)
	}
	if _, ok := metricStorage[key.bundle][key.ptransform]; !ok {
		metricStorage[key.bundle][key.ptransform] = make(map[name]userMetric)
	}
...
// gauge is a metric container for gauge values.
type gauge struct {
	mu sync.Mutex
	t  time.Time
	v  int64
...
func ClearBundleData(b string) {
	// No concurrency races since metricMu guards all access to metricStorage,
	// and the counter container sync.Maps are goroutine safe.
	metricMu.Lock()
	pts := metricStorage[b]
	for pt, m := range pts {
		for n := range m {
