...
func TryCoGroupByKey(s Scope, cols ...PCollection) (PCollection, error) {
	...
	if len(cols) < 1 {
		return PCollection{}, addContext(errors.New("need at least 1 pcollection"), s)
	}
	for i, in := range cols {
		if !in.IsValid() {
			return PCollection{}, addContext(errors.Errorf("invalid pcollection to CoGBK: index %v", i), s)
		}
	}
