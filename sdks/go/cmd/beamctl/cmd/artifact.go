...
func init() {
	...
	stageCmd.PersistentFlags().StringVarP(&token, "token", "e", "", "Session Storage token")
}
...
func stageFn(cmd *cobra.Command, args []string) error {
	...
	list, err := artifact.MultiStage(ctx, client, 10, files, token)
	if err != nil {
		return err
	}
	token, err := artifact.Commit(ctx, client, list, token)
	if err != nil {
		return err
	}
