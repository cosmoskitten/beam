...
def coerce_to_kv_type(element_type, label=None):
  ...
  if not element_type:
    ...
    if len(element_type.tuple_types) == 2:
      ...
      raise ValueError(
          "Tuple input to %s must be have two components. "
          "Found %s for %s" % (label, element_type))
  elif isinstance(input_type, AnyTypeConstraint):
    ...
    raise ValueError(
        "Input to %s must be of Tuple or Any type. "
        "Found %s for %s" % (label, element_type))
