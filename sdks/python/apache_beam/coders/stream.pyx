...
cdef class OutputStream(object):
  ...
  def __cinit__(self):
    self._size = 1024
    self.pos = 0
    self.data = <char*>libc.stdlib.malloc(self._size)
  ...
  cdef extend(self, size_t missing):
    while missing > self._size - self.pos:
      self._size *= 2
    self.data = <char*>libc.stdlib.realloc(self.data, self._size)
