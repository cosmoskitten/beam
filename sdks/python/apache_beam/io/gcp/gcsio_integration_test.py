...
class GcsIOIntegrationTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self._rewrite_responses = []
    self.gcsio._set_rewrite_response_callback(self._rewrite_callback)

  def tearDown(self):
    FileSystems.delete([self.gcs_tempdir + '/'])

  def _rewrite_callback(self, rewrite_response):
    if rewrite_response is None:
      return
    self._rewrite_responses.append(rewrite_response)
  ...
  # TODO: make this work in python postcommits
  @attr('IT')
  ...
  def test_copy_rewrite_token(self):
    ...
    if self.kms_key_name is None:
      raise unittest.SkipTest('--kms_key_name not specified')
    self.assertFalse(self._rewrite_responses)
    ...
    self.assertTrue(any(
        [not response.done for response in self._rewrite_responses]))
