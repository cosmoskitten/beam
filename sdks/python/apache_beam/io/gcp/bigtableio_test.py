...
from beam_bigtable.bigtable import _BigTableSource as BigTableSource
...
class BigtableSourceTest(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.project_id = 'grass-clump-479'
    cls.instance_id = 'python-write-2'
    #cls.table_id = DEFAULT_TABLE_PREFIX + str(uuid.uuid4())[:8]
    cls.table_id = 'testmillion7abb2dc3'
  ...
  def test_estimate_size(self):
    get_size = [k.offset_bytes for k in BigtableSourceTest.table.sample_row_keys()][-1]
  ...
  def test_split_rage_sized_subranges(self):
    ...
    count_all = len(list(self.bigtable.split_range_sized_subranges(sample_size_bytes,
                                                                    desired_bundle_size,
                                                                    ranges)))
  ...
  def test_get_sample_row_keys(self):
    print(list(self.bigtable.get_sample_row_keys()))
