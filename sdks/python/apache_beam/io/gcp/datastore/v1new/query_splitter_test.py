...
class QuerySplitterTest(QuerySplitterTestBase):
  ...
  def test_client_key_sort_key_ids(self):
    k1 = key.Key('kind', 2, project=self._PROJECT, namespace=self._NAMESPACE)
    k2 = key.Key('kind', 1, project=self._PROJECT, namespace=self._NAMESPACE)
    keys = [k1, k2]
    expected_sort = [k2, k1]
    keys.sort(key=query_splitter.client_key_sort_key)
    self.assertEqual(expected_sort, keys)
  ...
  def test_client_key_sort_key_ids_vs_names(self):
    ...
    k1 = key.Key('kind', '1', project=self._PROJECT, namespace=self._NAMESPACE)
    k2 = key.Key('kind', 2, project=self._PROJECT, namespace=self._NAMESPACE)
    keys = [k1, k2]
    expected_sort = [k2, k1]
    keys.sort(key=query_splitter.client_key_sort_key)
    self.assertEqual(expected_sort, keys)
