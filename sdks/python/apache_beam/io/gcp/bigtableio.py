...
class _BigTableSource(iobase.BoundedSource):
  ...
  def split(self, desired_bundle_size, start_position=None, stop_position=None):
    ...
    if self.beam_options['row_set'] is not None:
      ...
      full_size = [k.offset_bytes for k in self.get_sample_row_keys()][-1]
  ...
  def read(self, range_tracker):
    ...
    for therow in read_rows:
      try_claim = range_tracker.try_claim(therow.row_key)
      # if you get all the rows from bigtable
      # the last bundle get `b''` as stop_position
      # so, the try_claim get True and break the loop.
