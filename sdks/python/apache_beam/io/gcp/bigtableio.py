...
class _BigTableSource(iobase.BoundedSource):
  ...
  def range_split_fraction(self,
    ...
    sending the desired_bundle_size.
  ...
  def fraction_to_position(self, position, range_start, range_stop):
    ''' We use the ``fraction_to_position`` method in ``LexicographicKeyRangeTracker`` class
    to split a range into two chunks.
  ...
  def split_range_subranges(self,
                            ...
                            desired_bundle_size,
                            ranges):
    ''' This method split the range you get using the ``desired_bundle_size``
    as a limit size, It compares the size of the range and the ``desired_bundle size``
    if it is necessary to split a range, it uses the ``fraction_to_position`` method.
