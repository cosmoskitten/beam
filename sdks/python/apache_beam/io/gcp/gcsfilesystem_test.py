...
class GCSFileSystemTest(unittest.TestCase):
  ...
  @mock.patch('apache_beam.io.gcp.gcsio')
  def test_file_metadata_match(self, *unused_args):
    # Prepare mocks.
    gcsio_mock = mock.MagicMock()
    gcsio.GcsIO = lambda: gcsio_mock
    ...
    self.assertEqual(
        set(GCSFileSystem.match('gs://bucket/*')),
        expected_results)
  ...
  def test_rename_batch(self, *unused_args):
    ...
    # Issue batch rename.
    GCSFileSystem.rename(sources, destinations)
