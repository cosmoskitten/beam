...
class BigQueryReader(dataflow_io.NativeSourceReader):
  ...
  def __iter__(self):
    for rows, schema in self.client.run_query(
      ...
      for row in rows:
        if self.row_as_dict:
          logging.info('bigquery_reader_iter')
          logging.info(self.schema)
          logging.info(self.client.convert_row_to_dict(row, self.schema))
...
class RowAsDictJsonCoder(coders.Coder):
  ...
  def encode(self, table_row):
    ...
    try:
      logging.info('encode_row')
      logging.info(self.schema)
      logging.info(table_row)
      if isinstance(table_row, str):
        table_row = json.loads(table_row)
      # if python 3 and bytes value than encode it
  ...
  def decode(self, encoded_table_row):
    logging.info('decode_row')
    logging.info(self.schema)
    logging.info(encoded_table_row)
