...
class FileSystems(object):
  ...
  def join(basepath, *paths):
    ...
    """
    try:
      filesystem = get_filesystem(basepath)
    except Exception as e:
      raise BeamIOError('Enable to get the Filesystem', {basepath: e})
  ...
  def match(patterns, limits=None):
    ...
    if len(patterns) == 0:
      return []
    try:
      filesystem = get_filesystem(patterns[0])
    except Exception as e:
      raise BeamIOError('Enable to get the Filesystem', {patterns[0]: e})
  ...
  def copy(source_file_names, destination_file_names):
    ...
    if len(source_file_names) == 0:
      return
    try:
      filesystem = get_filesystem(source_file_names[0])
    except Exception as e:
      raise BeamIOError('Enable to get the Filesystem',
                        {source_file_names[0]: e})
  ...
  def delete(paths):
    ...
    if len(paths) == 0:
      return
    try:
      filesystem = get_filesystem(paths[0])
    except Exception as e:
      raise BeamIOError('Enable to get the Filesystem', {paths[0]: e})
