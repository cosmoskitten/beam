...
from apache_beam.io.external.avro_generic_coder import AvroGenericRecord
...
class XlangParquetIOTest(unittest.TestCase):
  def test_write_and_read(self):
    ...
    if not parquetio_jar:
      ...
      return
    test_pipeline.get_pipeline_options().view_as(SetupOptions).jar_packages = [
        parquetio_jar]
    port = test_pipeline.get_option("expansion_port")
    if not port:
