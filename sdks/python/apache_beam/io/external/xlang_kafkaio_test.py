...
class XlangKafkaIOTest(unittest.TestCase):
  def test_write_and_read(self):
    ...
    read_pipeline.get_pipeline_options().view_as(SetupOptions).jar_packages = [
        expansion_jar]
    ...
    try:
      ...
      write_pipeline.get_pipeline_options().view_as(SetupOptions).jar_packages \
        = [expansion_jar]
      write_pipeline.not_use_test_runner_api = True

      with write_pipeline as p:
        _ = p \
          | beam.Create(['abc', 'def', 'ghi']).with_output_types(str) \
          | beam.ExternalTransform(
              KAFKA_WRITE_URN,
              b'localhost:9092', address)
