...
class RangeTracker(object):
  ...
  **Consumed positions**
  ...
  def split_points(self):
    ...
    ** Determining consumed and remaining number of split points **
    ...
    consumed number of split points:
      ...
      * Any source read that has only claimed one split point and not set
        ``RangeTracker.done`` has 0 consumed split points: the first split point
        is the current split point and is still being processed. This condition
        holds independent of whether the input is splittable.
      ...
      * For a non-empty, finished source read (in which
        ...
        ``RangeTracker.done`` has been set to ``True``), the value returned must
      ...
      * For example (3): For any non-empty unsplittable input, the consumed
        number of split points is 0 until ``RangeTracker.`` is set to ``True``,
    ...
    * Any source read for which ``RangeTracker.done`` has not been set to True,
      remaining number of split points should not be zero, because this
    ...
    * A finished read, for which RangeTracker.done has been set to True,
    ...
    * For example (3): a source read for any non-empty unsplittable input,
      remaining number of split points is 1 until ``RangeTracker.done`` is set
      to ``True``, at which point remaining number of split points is 0.
  ...
  def set_split_points_remaining_callback(self, callback):
    ...
    By invoking this function, a BoundedSource can set a callback function that
    may get invoked by the RangeTracker to determine the number of remaining (
    unconsumed) split points. The callback function accepts a single parameter,
    a stop position for the BoundedSource (stop_position). If the split point
    currently being consumed by the BoundedSource is at position
  ...
  def set_done(self):
    """Marks that all records within the range has been consumed.
