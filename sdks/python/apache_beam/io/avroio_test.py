...
# pylint: disable=wrong-import-order, wrong-import-position, ungrouped-imp
try:
  ...
  # Never called but defined to enable the unittest to SKIP
  # instead of raising TypeError . The definition is only used
  # in python3 + avro, but this is deprecated, and thus skipped
  parse = lambda x: x
  ...
  def _write_data(self,
    ...
    with tempfile.NamedTemporaryFile(delete=False,
      ...
      writer(f, self.SCHEMA, all_records,
             codec=codec, sync_interval=sync_interval)
      self._temp_files.append(f.name)
    return f.name

  @unittest.skip('Skipping, using fastavro over avro somehow'
                 'results in different split_points_report variables,'
