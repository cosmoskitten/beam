...
def check_dofn_methods(actual):
  # The following is what the user would see through stdout by running the
  # sample. However, for the test we only need the elements produced by
  # `process` and `finish_bundle`, marked by a `*`, since those will be the
  # only actual elements in the output `PCollection`.
