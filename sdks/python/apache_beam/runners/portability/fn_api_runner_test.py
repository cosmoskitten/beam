...
class FnApiRunnerSplitTest(unittest.TestCase):
  ...
  def test_checkpoint(self):
    ...
    self.run_split_test(split_manager, list('abc'), element_counter)
  ...
  def run_split_test(self, split_manager, elements, element_counter=None):
  ...
  def test_checkpoint_sdf(self):
    ...
    elements = [2, 3]
    expected_groups = [[(e, k)] for e in elements for k in range(e)]
    self.run_sdf_split_test(
  ...
  def test_split_half_sdf(self):
    ...
    element_counter = ElementCounter()
    is_first_bundle = [True]  # emulate non-local
    ...
    def split_manager(num_elements):
      if is_first_bundle:
        ...
        self.verify_channel_split(split2, -1, 1, has_applications=True)
        self.verify_channel_split(split3, -1, 1, has_applications=True)
        breakpoint.clear()
  ...
  def test_split_crazy_sdf(self, seed=None):
    ...
    try:
      self.run_sdf_split_test(split_manager, elements, element_counter)
    except Exception:
      logging.error('test_split_crazy_sdf.seed = %s', seed)
      raise

  def run_sdf_split_test(
  ...
  def verify_channel_split(
      self, split_result, last_primary, first_residual, has_applications=False):
    ...
    if has_applications:
      self.assertTrue(split_result.primary_roots)
      self.assertTrue(split_result.residual_roots)
    else:
      self.assertFalse(split_result.primary_roots)
      self.assertFalse(split_result.residual_roots)
