...
class _PartitionBuffer(object):
  ...
  def partition(self, n):
    ...
    elif isinstance(self._inputs, _GroupingBuffer):
      return self._inputs.partition(n)
    else:
...
class ParallelBundleManager(BundleManager):
  ...
  def process_bundle(self, inputs, expected_outputs):
    ...
    # when timer is set, inputs len is 2.
    # example with timer:
    # {'Create/Read/Impulse': [b'xxx'],
    # 'ParDo(TimerDoFn)_timers_to_read_timer/Read': []}
    #
    # example without timer:
    # {'Create/Read/Impulse': [b'xxx']}
    #
    # We cannot split above inputs (with timer) when use GrpcHandler.
    # It's ok to split when not use GrpcHandler.
    #
    # TODO: what is a good way to know whether we can split inputs or not.
