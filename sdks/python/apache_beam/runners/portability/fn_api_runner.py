...
class FnApiRunner(runner.PipelineRunner):
    ...
    def expand_gbk(stages):
      ...
      for stage in stages:
        ...
        if transform.spec.urn == common_urns.primitives.GROUP_BY_KEY.urn:
          ...
          grouping_buffer = buffer_name(stage.name, kind='group')
    ...
    def sink_flattens(stages):
      ...
      for stage in stages:
        ...
        if transform.spec.urn == common_urns.primitives.FLATTEN.urn:
          ...
          buffer_id = buffer_name(transform.unique_name)
  ...
  def run_stage(
        ...
        if transform.spec.urn in (bundle_processor.DATA_INPUT_URN,
          ...
          for tag, si in payload.side_inputs.items():
            data_side_input[transform.unique_name, tag] = (
                buffer_name(transform.inputs[tag]),
    ...
    for (transform_id, tag), (buffer_id, si) in data_side_input.items():
      _, pcoll_id = split_buffer_name(buffer_id)
    ...
    def get_buffer(buffer_id):
      kind, name = split_buffer_name(buffer_id)
    ...
    while True:
      ...
      for transform_id, timer_writes in stage.timer_pcollections:
        ...
        written_timers = get_buffer(buffer_name(timer_writes, kind='timers'))
        if written_timers:
...
def buffer_name(name, kind='materialize'):
  return ('%s:%s' % (kind, name)).encode('utf-8')


def split_buffer_name(buffer_name):
   return buffer_name.decode('utf-8').split(':', 1)
