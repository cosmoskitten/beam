...
class FnApiRunner(runner.PipelineRunner):
  ...
  def run_pipeline(self, pipeline):
    ...
    self._bundle_repeat = pipeline._options.view_as(
        pipeline_options.DirectOptions).direct_runner_bundle_repeat
    self._profile_factory = profiler.Profile.factory_from_options(
  ...
  def maybe_profile(self):
    if self._profile_factory:
      ...
      profiler = self._profile_factory(profile_id, time_prefix='')
    else:
      profiler = None
    ...
    if profiler:
      ...
      print('CPU Profile written to', path)
      try:
        import gprof2dot
