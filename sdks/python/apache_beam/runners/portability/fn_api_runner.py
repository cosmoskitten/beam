...
class FnApiRunner(runner.PipelineRunner):
  ...
  def _run_stage(self,
    ...
    # import random
    # self._num_workers = random.randint(1, 10)
    # logging.info('***************** %s', self._num_workers)
...
class WorkerHandlerManager(object):
  ...
  def get_worker_handlers(self, environment_id, num_workers):
    ...
    if num_workers > self._grpc_server.max_workers:
      raise RuntimeError('gRPC servers are running with %s threads, we cannot '
                         'attach %s workers to it.' % (
          self._grpc_server.max_workers, num_workers))
