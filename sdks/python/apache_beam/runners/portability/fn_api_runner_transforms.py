...
def replace_transforms(components, stages):
  transforms_copy = dict(components.transforms)
  components.transforms.clear()
  parents = {
      child: parent
      for parent, proto in transforms_copy.items()
      for child in proto.subtransforms
  }

  def add_parent(child, parent):
    if parent not in components.transforms:
      components.transforms[parent].CopyFrom(transforms_copy[parent])
