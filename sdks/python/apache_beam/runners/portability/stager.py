...
class Stager(object):
  ...
  def stage_job_resources(self,
    ...
    jar_package = options.view_as(DebugOptions).lookup_experiment('jar_package')
    if jar_package is not None:
      resources.extend(
          self._stage_jar_packages(
              jar_package, staging_location,
              temp_dir=temp_dir))
  ...
  def _stage_jar_packages(self, jar_package, staging_location, temp_dir):
    ...
    local_packages = []
    for package in jar_package.split(':'):
      if not os.path.basename(package).endswith('.jar'):
        raise RuntimeError(
            'The --jar_package option expects a full path ending with ".jar" '
            'instead of %s' % package)
      ...
      if not os.path.isfile(package):
        if Stager._is_remote_path(package):
          ...
          raise RuntimeError(
              'The file %s cannot be found. It was specified in the '
              '--jar_package command line option.' % package)
      else:
