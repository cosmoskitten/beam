...
from apache_beam import error, pipeline, pvalue
from iobase import (
  ...
  def test_repr_method(self):
    class FakeSource(NativeSource):
      ...
      def __init__(self, table=None, dataset=None, project=None, query=None,
        ...
        self.use_std_sql = use_standard_sql
        self.flatten_results = flatten_results

    fake_source = FakeSource()
    self.assertEqual(
        fake_source.__repr__(),
        '<FakeSource validate=False, use_std_sql=False, flatten_results=True>'
    )


class TestNativeSourceReader(unittest.TestCase):

  def setUp(self):
    self.native_source_reader = NativeSourceReader()

  def test_enter_method(self):
    self.assertRaises(NotImplementedError, self.native_source_reader.__enter__)

  def test_exit_method(self):
    with self.assertRaises(NotImplementedError):
      self.native_source_reader.__exit__(None, None, None)

  def test_iter_method(self):
    self.assertRaises(NotImplementedError, self.native_source_reader.__iter__)

  def test_returns_window_values_property(self):
    self.assertEqual(self.native_source_reader.returns_windowed_values, False)

  def test_get_progress(self):
    self.assertEqual(self.native_source_reader.get_progress(), None)

  def test_request_dynamic_split(self):
    request = DynamicSplitRequest(ReaderProgress(None))
    self.assertEqual(
        self.native_source_reader.request_dynamic_split(request),
        None
    )
  ...
  def test_repr_method(self):
    class FakeSink(NativeSink):
      ...
      def __init__(self, table, dataset=None, project=None, schema=None,
                   create_disposition='create', write_disposition='write',
    ...
    fake_sink = FakeSink(None)
    self.assertEqual(
        fake_sink.__repr__(),
        '<FakeSink ["create_disposition=\'create\'", ' +
        '"write_disposition=\'write\'", \'validate=False\']>'
    )


class TestNativeSinkWriter(unittest.TestCase):

  def setUp(self):
    self.native_sink_writer = NativeSinkWriter()

  def test_enter_method(self):
    self.assertRaises(NotImplementedError, self.native_sink_writer.__enter__)

  def test_exit_method(self):
    with self.assertRaises(NotImplementedError):
      self.native_sink_writer.__exit__(None, None, None)

  def test_takes_windowed_values_property(self):
    self.assertEqual(self.native_sink_writer.takes_windowed_values, False)

  def test_Write_method(self):
    self.assertRaises(NotImplementedError, self.native_sink_writer.Write, None)
...
class Test_NativeWrite(unittest.TestCase):
  ...
  def test_expand_method(self):
    pcoll = pvalue.PCollection(pipeline=pipeline.Pipeline())
    self.assertEqual(type(self.native_write.expand(pcoll)), pvalue.PDone)
