...
class DataflowRunner(PipelineRunner):
  ...
  def run_ParDo(self, transform_node):
    ...
    for ix, side_pval in enumerate(transform_node.side_inputs):
      ...
      label_renames['side%d' % ix] = si_label
      assert si_label in named_inputs
    ...
    if (apiclient._use_fnapi(transform_node.inputs[0].pipeline._options)
      ...
      # Validate side input labels in proto.
      for si_label in si_labels.values():
        assert si_label in transform_proto.inputs
