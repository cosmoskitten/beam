...
class DataflowRunner(PipelineRunner):
  ...
  def run_ParDo(self, transform_node, options):
    ...
    main_output_tag = transform.main_output_tag

    # Some SDKs do not work correctly if output tags are modified. So for
    ...
    outputs.append(
        {PropertyNames.USER_NAME: (
            '%s.%s' % (transform_node.full_label, PropertyNames.OUT)),
         PropertyNames.ENCODING: step.encoding,
         PropertyNames.OUTPUT_NAME: main_output_tag if (
             external_transform and main_output_tag) else PropertyNames.OUT})
