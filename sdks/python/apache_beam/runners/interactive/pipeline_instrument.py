...
class PipelineInstrument(object):
  ...
  def _write_cache(self, pcoll):
    ...
    if not self._cache_manager.exists('full', key):
      ...
      pcoll | '{}{}'.format(WRITE_CACHE, key) >> cache.WriteCache(
  ...
  def _read_cache(self, pcoll):
    ...
    if self._cache_manager.exists('full', key):
      if key not in self._cached_pcoll_read:
        ...
        pcoll_from_cache = (
            self._pipeline | '{}{}'.format(READ_CACHE, key) >> cache.ReadCache(
            self._cache_manager, key))
  ...
  def _debug_appliedptransform(self, transform_node):
    ...
    [logging.debug('  %s', inp) for inp in transform_node.inputs]
    logging.debug('parts: ')
    [logging.debug('  %s', part) for part in transform_node.parts]
    logging.debug('outputs: ')
    [logging.debug('  %s', ot) for ot in transform_node.outputs]
