...
class InteractiveEnvironment(object):
  ...
  def watching(self):
    ...
    Internally used by iBeam. No backward compatibility guaranteed. Each entry
    in the list represents the variable defined within a watched watchable.
    Currently, each entry holds a list of pairs. The format might change in the
    future to hold more metadata. Duplicated pairs are allowed. And multiple
    paris can have the same variable name as the first while having different
    variable values as the second since variables in different scopes can have
    the same name.
    """
