...
class FakeSubscriberClient(object):
  ...
  def create_subscription(self, name, topic, *args, **kwargs):
    ...
    subscription_proto = pubsub.types.Subscription()
    subscription_proto.name = name
    subscription_proto.topic = topic
  ...
  def create_snapshot(self, name, subscription, *args, **kwargs):
    ...
    snapshot = pubsub.types.Snapshot()
    snapshot.name = name
...
@mock.patch(
    "apache_beam.runners.interactive.caching.streambasedcache.pubsub",
...
class PubSubBasedCacheTest(ExtraAssertionsMixin, unittest.TestCase):
  ...
  def test_read(self, mock_pubsub):
    ...
    mock_read_to_queue = mock.MagicMock(side_effect=lambda: mock_queue_context)
...
@mock.patch(
    "apache_beam.runners.interactive.caching.streambasedcache.pubsub",
    new_callable=FakePubSub)
class PubSubBasedCacheCoderTest(filebasedcache_test.CoderTestBase,
                                unittest.TestCase):
  ...
  @parameterized.expand([(write_fn.__name__, write_fn, data)
                         for data in filebasedcache_test.GENERIC_TEST_DATA
                         for write_fn in [
