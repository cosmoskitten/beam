...
def read_through_pipeline(cache,
  ...
  temp_location = pubsub.PublisherClient.topic_path(
      os.getenv("PUBSUB_PROJECT_ID", "test-project"),
      "read-test-" + uuid.uuid4().hex)
  temp_cache = streambasedcache.PubSubBasedCache(temp_location, single_use=True)
  p = Pipeline(
      runner=BundleBasedDirectRunner(), options=PipelineOptions(streaming=True))
  # yapf: disable
  _ = (
      p
      | "Read" >> cache.reader()
  ...
  pr = p.run()
  try:
    for element in itertools.islice(temp_cache.read(timeout=timeout), limit):
      if return_timestamp:
        yield element
      else:
        yield element[1]
...
class PubSubBasedCacheRoundtripTest(RoundtripTestBase, unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.location = pubsub.PublisherClient.topic_path(
        os.getenv("PUBSUB_PROJECT_ID", "test-project"), topic_name)
...
class FastPubSubBasedCacheRoundtripTest(
  ...
  def check_roundtrip(self, write_fn, read_fn, data):
    ...
    write_fn(cache, data)
    data_out = list(read_fn(cache, limit=len(data)))
