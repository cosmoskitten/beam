...
class PubSubBasedCache(StreamBasedCache):
  ...
  def read(self, timeout=None, return_timestamp=False, **reader_kwargs):
    self._assert_topic_exists()
    self._assert_read_valid()
    ...
    def iter_parsed_messages():
      while True:
        try:
          yield parsed_message_queue.get(timeout=timeout)
        except queue.Empty:
          return
    ...
    try:
      for timestamp, parsed_message in iter_parsed_messages():
        if return_timestamp:
      ...
      for element in elements:
        ...
        if do_fn.with_attributes:
          element_bytes = PubsubMessage(
              element_bytes, {a: element[a] for a in with_attributes})
  ...
  def clear(self):
    remove_topic_and_descendants(self.location)
  ...
  def _get_or_create_subscription(self, subscription_path=None):
