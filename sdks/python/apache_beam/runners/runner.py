  ...
  def cache_output(self, transform, tag_or_value, value=None):
    ...
    try:
     self._cache[
        ...
        pickler.dumps(value)
        self.test_pickle(value)
        raise

  def test_pickle(self, xThing,lTested = []):
    import pickle
    ...
    try:
      attrs = xThing.__dict__.keys()
    except:
      attrs = []
    for s in attrs:
        if s.startswith('_') and False:
        ...
        if type(xAttribute) == dict:
            ...
            for k in xAttribute:
                lTested = self.test_pickle(xAttribute[k])
                continue
        ...
        try:
            ...
            for x in xAttribute:   
                lTested = self.test_pickle(x,lTested)
        ...
        try:
            ...
            lTested = self.test_pickle(xAttribute,lTested)
            continue
