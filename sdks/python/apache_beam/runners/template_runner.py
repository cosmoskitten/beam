# template_runner -- a modified copy of direct_runner
# Replace DirectPipelineRunner with TemplatingDataflowPipelineRunner
# It runs
...
"""TemplatingDataflowPipelineRunner, executing on the local machine.

The TemplatingDataflowPipelineRunner class implements what is called in Dataflow
...
class TemplatingDataflowPipelineRunner(PipelineRunner):
	pass
  # def __init__(self, inner_runner):
  #   self.inner_runner = inner_runner

  # def apply(self, transform, input):
  # 	return self.inner_runner.apply(transform, input)

  # def run(self, pipeline):
  #   return self.inner_runner.run(pipeline)

  # # option 2: Remove the extra stuff from dataflow runner
  # def run(self, pipeline):
  ...
  #   # If --dataflowJobFile: (instead of if --runTemplate), save job:
  #   # 1) Serialize -- how do you do that? Something like json.dumps?
  #   # 2) Write it to remote location given by --dataflowJobFile?
  #   # add hook?
  #   # Is the hook what makes it know whether the flag --run-template is set?

  #   # if dataflow job file present?
  #   debug_options = pipeline.options.view_as(DebugOptions)
  ...
  #   if debug_options.dataflow_job_file:
  #     self.result = DataflowPipelineResult(
  #         self.dataflow_client.create_template_job(self.job, debug_options.dataflow_job_file))
  #   else:
  #   	# error: it needs to be set to get here
...
#   def run(self, pipeline):
#   super(TemplatingDataflowPipelineRunner, self).run(pipeline)
...
# class EagerPipelineRunner(TemplatingDataflowPipelineRunner):
...
# class DiskCachedPipelineRunner(TemplatingDataflowPipelineRunner):
#   """A TemplatingDataflowPipelineRunner that uses a disk backed cache.
...
#     return super(TemplatingDataflowPipelineRunner, self).run(pipeline)
#   finally:
#     del self._cache
