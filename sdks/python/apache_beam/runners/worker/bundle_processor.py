...
class BundleProcessor(object):
  ...
  def __init__(
    ...
    self.ops, self._bundle_finalizers = self.create_execution_tree(self.process_bundle_descriptor)
  ...
  def create_execution_tree(self, descriptor):
    ...
    def get_bundle_finalizer_operaions(transforms, operations_dict):
      bundle_finalizers = []
      for transform_id, transfrom_proto in transforms.items():
        if transform_proto.spec.urn == common_urns.primitives.PAR_DO.urn and proto_utils.parse_Bytes(
              transform_proto.spec.payload,
              beam_runner_api_pb2.ParDoPayload).requests_finalization:
          bundle_finalizers.append(operations_dict[transform_id])
    ...
    bundle_finalizers = get_bundle_finalizer_operaions(descriptor.transforms,operations_dict)
  ...
  def process_bundle(self, instruction_id):
    ...
    try:
      ...
      return [
          ...
          for op, residual in execution_context.delayed_applications], self.requires_finalization
