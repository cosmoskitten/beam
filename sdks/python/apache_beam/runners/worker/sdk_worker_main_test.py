...
class SdkWorkerMainTest(unittest.TestCase):
  ...
  def test_status_server(self):
    ...
    thread = threading.Thread(target=functools.partial(status_server.start, started_callback=callback))
    thread.daemon = True
    thread.start()
    condition.acquire()
    # Wait for maximum 10 sec before the server is started. Though the server should not take this long to start.
    condition.wait(10)
    condition.release()
    ...
    def wrapped_method_for_test():
        conn = httplib.HTTPConnection(host=status_server.httpd.server_name, port=status_server.httpd.server_port)
        conn.request("GET", "/")
