...
class MeasureTime(beam.DoFn):
  def __init__(self, namespace):
    ...
    self.runtime = Metrics.distribution(self.namespace, METRICS['distributions']['RUNTIME_METRIC'])
...
def count_bytes(f):
  def repl(*args):
    namespace = args[2]
    counter = Metrics.counter(namespace, METRICS['counters']['BYTES_COUNTER_LABEL'])
    element = args[1]
    _, value = element
