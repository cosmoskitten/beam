...
class WindowFn(object):
  ...
  def from_runner_api(cls, fn_proto, context):
    ...
    return constructor(
        _unpack_any(fn_proto.spec.parameter, parameter_type), context)
  ...
  def to_runner_api(self, context):
    ...
    return beam_runner_api_pb2.FunctionSpec(
        spec=beam_runner_api_pb2.UrnWithParameter(
            urn=urn,
            parameter=_pack_any(typed_param)))
...
def _unpack_any(any_msg, msg_class):
...
class FixedWindows(WindowFn):
  ...
  def to_runner_api_parameter(self, context):
    return (urns.FIXED_WINDOWS_FN,
            _pack_struct(size=self.size.micros,
...
class Sessions(WindowFn):
  ...
  def to_runner_api_parameter(self, context):
    return (urns.SESSION_WINDOWS_FN,
            _pack_struct(gap_size=self.gap_size.micros))
