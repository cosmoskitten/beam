...
class InterfaceTest(unittest.TestCase):
  ...
  def test_get_on_timer_methods(self):
    class StatefulDoFnWithTimer(DoFn):
      BUFFER_STATE = BagStateSpec('buffer', BytesCoder())
      EXPIRY_TIMER_1 = TimerSpec('expiry1', TimeDomain.WATERMARK)
      EXPIRY_TIMER_2 = TimerSpec('expiry2', TimeDomain.WATERMARK)
    ...
    dofn = StatefulDoFnWithTimer()
    self.assertEqual(
        sorted(UserStateUtils.get_on_timer_methods(dofn).items()),
        sorted([(StatefulDoFnWithTimer.EXPIRY_TIMER_1, dofn.on_expiry_1),
                (StatefulDoFnWithTimer.EXPIRY_TIMER_2, dofn.on_expiry_2),]))
  ...
  def test_validation_typos(self):
    ...
    dofn = StatefulDoFnWithTimerWithTypo1()
    with self.assertRaisesRegexp(
        ValueError, "Duplicate on_timer method for timer of name 'expiry1'."):
      self._validate_dofn(dofn)

    # (2) Here, the user mistakenly used the same callback name and overwrote
    # the first on_expiry_1 callback. We can only provide a warning in this
    # case.
    class StatefulDoFnWithTimerWithTypo2(DoFn):
      ...
      # Note that we mistakenly reuyse the "on_expiry_2" name; this is valid
    ...
    validation_warnings = UserStateUtils.validate_stateful_dofn(dofn)
    self.assertEqual(
        validation_warnings,
        ['In DoFn StatefulDoFnWithTimerWithTypo2: timer spec '
         'TimerSpec(expiry1) does not have a corresponding @on_timer '
         'callback.'])
