...
class CombineTest(unittest.TestCase):
  ...
  def test_combine_per_key_top_display_data(self):
    def individual_test_per_key_dd(combineFn):
      ...
      expected_items = [
          DisplayDataItemMatcher.matches_kv('combineFn', combineFn.__class__),
          DisplayDataItemMatcher.matches_kv('n', combineFn._n)]
  ...
  def test_combine_sample_display_data(self):
    def individual_test_per_key_dd(sampleFn, args, kwargs):
      ...
      for transform in trs:
        dd = DisplayData.create_from(transform)
        expected_items = [
            DisplayDataItemMatcher.matches_kv('fn', sampleFn.fn.__name__),
            DisplayDataItemMatcher.matches_kv('combineFn',
                                              transform.fn.__class__)]
        if len(args) > 0:
          expected_items.append(
              DisplayDataItemMatcher.matches_kv('args', str(args)))
        if len(kwargs) > 0:
          expected_items.append(
              DisplayDataItemMatcher.matches_kv('kwargs', str(kwargs)))
        hc.assert_that(dd.items, hc.contains_inanyorder(*expected_items))
  ...
  def test_combine_globally_display_data(self):
    ...
    dd = DisplayData.create_from(transform)
    expected_items = [
        DisplayDataItemMatcher.matches_kv('combineFn', combine.Smallest),
        DisplayDataItemMatcher.matches_kv('n', 5)]
    hc.assert_that(dd.items, hc.contains_inanyorder(*expected_items))
  ...
  def test_basic_combiners_display_data(self):
    ...
    dd = DisplayData.create_from(transform)
    expected_items = [
        DisplayDataItemMatcher.matches_kv('combineFn', combine.TupleCombineFn),
        DisplayDataItemMatcher.matches_kv('combiners',
