  ...
  def _add_argparse_args(cls, parser):
    ...
    parser.add_argument(
        ...
        ('Local path to a Python package file. The file is expected to be (1) '
         ...
         '(\'.tar.gz\') or (3) a wheel file (\'.whl\') which can be installed '
         'using the "pip install" command of the standard pip package. '
         'Multiple --extra_package options can be specified if more than one '
         'package is needed. During job submission, the files will be staged '
         'in the staging area (--staging_location option) and the workers will '
         'install them in same order they were specified on the command line.'))
