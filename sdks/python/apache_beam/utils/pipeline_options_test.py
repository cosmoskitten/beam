...
class PipelineOptionsTest(unittest.TestCase):
  ...
  TEST_CASES = [
      {'flags': ['--num_workers', '5'],
       ...
       'display_data': [DisplayDataItemMatcher.matches_kv('num_workers', 5)]},
      {
          'flags': [
          ...
          'display_data': [
              DisplayDataItemMatcher.matches_kv('profile_cpu',
                                                True),
              DisplayDataItemMatcher.matches_kv('profile_location',
                                                'gs://bucket/')]
      },
      {'flags': ['--num_workers', '5', '--mock_flag'],
       'expected': {'num_workers': 5, 'mock_flag': True, 'mock_option': None},
       'display_data': [
           DisplayDataItemMatcher.matches_kv('num_workers', 5),
           DisplayDataItemMatcher.matches_kv('mock_flag', True)]
      },
      {'flags': ['--mock_option', 'abc'],
       'expected': {'mock_flag': False, 'mock_option': 'abc'},
       'display_data': [
           DisplayDataItemMatcher.matches_kv('mock_option', 'abc')]
      },
      {'flags': ['--mock_option', ' abc def '],
       'expected': {'mock_flag': False, 'mock_option': ' abc def '},
       'display_data': [
           DisplayDataItemMatcher.matches_kv('mock_option', ' abc def ')]},
      {'flags': ['--mock_option= abc xyz '],
       'expected': {'mock_flag': False, 'mock_option': ' abc xyz '},
       'display_data': [
           DisplayDataItemMatcher.matches_kv('mock_option', ' abc xyz ')]
      },
