...
public class FileIOChannelFactoryTest {
  ...
  public void testMatchNone() throws Exception {
    ...
    assertThat(factory.match(PathUtils.resolve(temporaryFolder.getRoot().getPath(), "b") + "*"),
        containsInAnyOrder(expected.toArray(new String[expected.size()])));
  }
  ...
  public void testMatchUsingExplicitPath() throws Exception {
    ...
    assertThat(factory.match(PathUtils.resolve(temporaryFolder.getRoot().getPath(), "a")),
        containsInAnyOrder(expected.toArray(new String[expected.size()])));
  }
  ...
  public void testMatchMultipleWithSubdirectoryExpansion() throws Exception {
    ...
    // Windows doesn't like resolving paths with * in them, so the ** is appended after resolve.
    assertThat(
        factory.match(PathUtils.resolve(temporaryFolder.getRoot().getPath(), "a") + "**"),
  ...
  public void testMatchWithoutParentDirectory() throws Exception {
    String pattern =
        PathUtils.resolve(temporaryFolder.getRoot().getPath(), "non_existing_dir", "*");
  ...
  public void testGetSizeBytesForNonExistentFile() throws Exception {
    ...
    factory.getSizeBytes(
        PathUtils.resolve(temporaryFolder.getRoot().getPath(), "non-existent-file"));
  }
}
