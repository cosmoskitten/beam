...
public class WatchTest implements Serializable {
                        ...
                        new PollFn<String, String>() {
                          ...
                          public PollResult<String> apply(TimestampedValue<String> input) {
                            ...
                            return PollResult.complete(
                                time,
                                Arrays.asList(
                                    input.getValue() + ".foo", input.getValue() + ".bar"));
                          }
                        })
  ...
  public void testSinglePollWithManyResults() {
    // More than the default 10000 elements per checkpoint
    final long numResults = 30000;
            ...
            .apply(
                Watch.growthOf(
                        new PollFn<String, KV<String, Integer>>() {
                          @Override
                          public PollResult<KV<String, Integer>> apply(
                              TimestampedValue<String> input) {
        ...
        .satisfies(
            new SerializableFunction<Iterable<Integer>, Void>() {
              ...
              public Void apply(Iterable<Integer> input) {
                assertEquals(numResults, Sets.newHashSet(input).size());
                return null;
              }
            });
            ...
            .apply(
                Watch.growthOf(
                        new TimedPollFn<String, Integer>(
                            ...
                            standardSeconds(5) /* timeToDeclareOutputFinal */,
                            standardSeconds(10) /* timeToFail */))
  ...
  private static class TimedPollFn<InputT, OutputT> implements PollFn<InputT, OutputT> {
    ...
    @Override
    public PollResult<OutputT> apply(TimestampedValue<InputT> input) {
  ...
  public void testGrowthTrackerCheckpointEmpty() {
    ...
    GrowthTracker<String> tracker = new GrowthState<String>(never()).newTracker();
  ...
  public void testGrowthTrackerCheckpointNonEmpty() {
    ...
    GrowthTracker<String> residualTracker = tracker.checkpoint().newTracker();
    GrowthTracker<String> primaryTracker = tracker.currentRestriction().newTracker();
    ...
    assertTrue(primaryTracker.isOutputFinal());
    ...
    assertFalse(residualTracker.isOutputFinal());
    ...
    assertTrue(tracker.isOutputFinal());
  ...
  public void testGrowthTrackerOutputFullyBeforeCheckpointComplete() {
    ...
    assertFalse(residualTracker.hasPending());
    assertTrue(residualTracker.isOutputFinal());
    // No more pending elements in residual restriction, but poll watermark still holds.
