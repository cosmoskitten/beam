...
public class DoFnInvokersTest {
  ...
  @Mock private ArgumentProvider<String, String> argumentProvider;
  ...
  public void setUp() {
    ...
    when(argumentProvider.window()).thenReturn(mockWindow);
    when(argumentProvider.inputProvider()).thenReturn(mockInputProvider);
    when(argumentProvider.outputReceiver()).thenReturn(mockOutputReceiver);
    when(argumentProvider.windowingInternals()).thenReturn(mockWindowingInternals);
    when(argumentProvider.processContext(Matchers.<DoFn>any())).thenReturn(mockProcessContext);
  }

  private ProcessContinuation invokeProcessElement(DoFn<String, String> fn) {
    return DoFnInvokers.invokerFor(fn).invokeProcessElement(argumentProvider);
  }
  ...
  public void testDoFnWithState() throws Exception {
    ...
    when(argumentProvider.state(stateId)).thenReturn(mockState);
  ...
  public void testDoFnWithTimer() throws Exception {
    ...
    when(argumentProvider.timer(timerId)).thenReturn(mockTimer);
  ...
  public void testSplittableDoFnDefaultMethods() throws Exception {
    ...
    assertEquals(
        ProcessContinuation.stop(), invoker.invokeProcessElement(argumentProvider));
  }
  ...
  public void testOldDoFnProcessElement() throws Exception {
    new DoFnInvokers.OldDoFnInvoker<>(mockOldDoFn)
        .invokeProcessElement(argumentProvider);
