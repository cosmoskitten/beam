          ...
          new DoFn<KV<String, Integer>, Integer>() {
            ...
            public void processElement(@TimerId(timerId) Timer timer) {
              try {
                ...
                List<String> expectedSubstrings = Arrays.asList("relative timers", "processing time");
                expectedSubstrings.forEach(str -> Preconditions.checkState(message.contains(str),
                    "Pipeline didn't fail with the expected strings: %s", expectedSubstrings));
              }
            }
