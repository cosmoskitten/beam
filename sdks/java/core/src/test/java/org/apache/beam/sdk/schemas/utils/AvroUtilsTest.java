...
public class AvroUtilsTest {
  ...
  @Test
  public void testAsSchemaPCollection() {
    ...
    PCollection<GenericRecord> schemaAwareRecords =
        AvroUtils.asSchemaPCollection(records, GenericRecord.class, schema);
    assertTrue(schemaAwareRecords.hasSchema());

    AvroGeneratedUser user = new AvroGeneratedUser("foo", 42, "green");
    PCollection<AvroGeneratedUser> users = pipeline.apply(Create.of(user));
    assertFalse(users.hasSchema());
    PCollection<AvroGeneratedUser> schemaAwareUsers =
        AvroUtils.asSchemaPCollection(users, AvroGeneratedUser.class, null);
    assertTrue(schemaAwareUsers.hasSchema());
  }
