...
public class DisplayDataEvaluator {
  ...
  public <InputT> Set<DisplayData> displayDataForPrimitiveTransforms(
    ...
    return visitor.getDisplayDatas();
  }

  /**
   * Visits {@link PTransform PTransforms} in a pipeline, and collected {@link DisplayData} for
  ...
  private static class PrimitiveDisplayDataPTransformVisitor implements Pipeline.PipelineVisitor {
    ...
    private final Set<DisplayData> displayDatas;
    private boolean shouldRecord = false;

    PrimitiveDisplayDataPTransformVisitor(PTransform root) {
      this.root = root;
      this.displayDatas = Sets.newHashSet();
    }

    Set<DisplayData> getDisplayDatas() {
      return displayDatas;
    }
    ...
    public void visitTransform(TransformTreeNode node) {
      if (shouldRecord) {
        displayDatas.add(DisplayData.from(node.getTransform()));
      }
    }
    @Override
