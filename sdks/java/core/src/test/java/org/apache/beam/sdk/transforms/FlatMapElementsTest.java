...
public class FlatMapElementsTest implements Serializable {
   ...
   */
  private static class GenericIterableFunction<T> extends SimpleFunction<T, Iterable<T>> {
    @Override
   ...
   */
  @Test
  public void testTypeVariableCoderPropagation() throws Exception {
    Pipeline pipeline = TestPipeline.create();
    PCollection<Integer> output = pipeline
        .apply(Create.of(1, 2, 3))

        // This is the function that needs to propagate the input T to output T
        .apply("Generic Iterable", MapElements.via(new GenericIterableFunction<Integer>()))
