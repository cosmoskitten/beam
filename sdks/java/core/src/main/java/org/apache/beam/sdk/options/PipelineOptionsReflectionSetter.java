...
public class PipelineOptionsReflectionSetter {
  ...
  public static Class<? extends PipelineOptions> getPipelineOptionsInterface(
    ...
    if (options.getClass().getInterfaces().length != 1) {
      throw new IllegalArgumentException("The input must implement only one interface");
    }
      ...
      switch (closestMatches.size()) {
        case 0:
          throw new IllegalArgumentException(
              String.format("Class %s missing a property named '%s'.", optionsClass, key));
        case 1:
          throw new IllegalArgumentException(
              String.format(
                  "Class %s missing a property named '%s'. Did you mean '%s'?",
                  optionsClass, key, Iterables.getOnlyElement(closestMatches)));
        default:
          throw new IllegalArgumentException(
              String.format(
                  "Class %s missing a property named '%s'. Did you mean one of %s?",
                  optionsClass, key, closestMatches));
      }
    }
    return pmap.get(key);
  }
