 ...
 *
 * <p>Consuming transforms such as {@link MapElements.MapWithExceptions} follow the general
 * pattern of taking in a user-defined exception handler of type {@code
 ...
 * was raised. This handler is responsible for producing some output element that
 * captures relevant details of the error and can be encoded as part of an error output
 * {@link PCollection}. Transforms can then package together their output and error collections
 * in a {@link WithExceptions.Result} that avoids users needing to interact with {@code TupleTag}s
 * and indexing into a {@link PCollectionTuple}.
 *
 * Exception handlers can narrow their scope by rethrowing the passed {@link
...
public class WithExceptions {
   ...
   *
   * <p>Extends {@link SimpleFunction} so that full type information is captured. Map and {@link
   * KV} coders are well supported by Beam, so coder inference can be successfully applied if
   * the consuming transform passes type information to the error collection's {@link TupleTag}.
   *
