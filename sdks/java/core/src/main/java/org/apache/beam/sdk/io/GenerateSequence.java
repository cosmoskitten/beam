...
public abstract class GenerateSequence extends PTransform<PBegin, PCollection<Long>> {
  ...
  abstract static class Builder
    ...
    public GenerateSequence buildExternal(External.ExternalConfiguration config) {
      ...
      Builder builder = GenerateSequence.from(config.start).toBuilder();
      if (config.stop != null) {
        builder.setTo(config.stop);
      }
      if (config.period != null) {
        builder.setPeriod(Duration.millis(config.period));
      }
      if (config.maxReadTime != null) {
        builder.setMaxReadTime(Duration.millis(config.maxReadTime));
      }
      if (config.elements_per_period != null) {
        builder.setElementsPerPeriod(config.elements_per_period);
      }
