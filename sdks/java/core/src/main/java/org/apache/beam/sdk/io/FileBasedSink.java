...
public abstract class FileBasedSink<T> implements Serializable, HasDisplayData {
  ...
  public FileBasedSink(
      String baseOutputDirectory, FilenamePolicy filenamePolicy) {
    this(
        StaticValueProvider.of(
            FileSystems.matchNewResource(baseOutputDirectory, true /* isDirectory */)),
        filenamePolicy,
  ...
  public FileBasedSink(
    ...
    this.baseOutputDirectoryProvider = baseOutputDirectoryProvider;
  ...
  public abstract static class FileBasedWriter<T> {
    ...
    private void open(String uId,
      ...
      LOG.debug("Opening {} for write with MIME type {}.", outputFile, mimeType);
      WritableByteChannel tempChannel = FileSystems.create(outputFile, mimeType);
      try {
        channel = getWriteOperation().getSink().writableByteChannelFactory.create(tempChannel);
      } catch (Exception e) {
