...
public class CompressedSource<T> extends FileBasedSource<T> {
  ...
  public static class CompressedReader<T> extends FileBasedReader<T> {
    ...
    // Compressed sources are unsplittable: returns the offset in the input stream that has been
    // read by the input. These positions are likely to be coarse-grained (in the event of
    // buffering) and over-estimates (because they reflect the number of bytes read to produce an
    // element, not its start) but both of these provide better progress information than e.g.,
    // reporting the start of the file.
    @Override
