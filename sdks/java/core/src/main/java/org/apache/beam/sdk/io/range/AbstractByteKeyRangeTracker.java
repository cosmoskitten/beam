...
public abstract class AbstractByteKeyRangeTracker implements RangeTracker<ByteKey> {
  ...
  public synchronized boolean tryReturnRecordAt(boolean isAtSplitPoint, ByteKey recordStart) {
    ...
    if (position == null) {
      LOG.info(
          ...
          getStartPosition(),
          recordStart);
      setStartingPosition(recordStart);
    }
  ...
  public synchronized boolean trySplitAtPosition(ByteKey splitPosition) {
    ...
    if (!containsKey(splitPosition)) {
      LOG.warn(
          "{}: Rejecting split request at {} because it is not within this tracker.",
          this,
          splitPosition);
      return false;
    }
   ...
   */
  protected abstract void setEndPosition(ByteKey end);
   ...
   */
  protected abstract void setStartingPosition(ByteKey start);
   ...
   */
  protected abstract boolean containsKey(ByteKey key);
