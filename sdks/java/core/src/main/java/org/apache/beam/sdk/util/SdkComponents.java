...
public class SdkComponents {
  ...
  private final BiMap<PCollection<?>, String> collectionIds;
  ...
  private SdkComponents() {
    ...
    this.collectionIds = HashBiMap.create();
   ...
   * provided ID has already been used to represent a {@link PCollection}, this method will throw an
   ...
   * already present, returning the id of the {@link WindowingStrategy} in this
   * {@link SdkComponents}.
   */
  ...
  public String getPCollectionId(PCollection<?> collection) {
    checkArgument(
        collectionIds.containsKey(collection),
        "Unknown %s %s",
        PCollection.class.getSimpleName(),
        collection);
    return collectionIds.get(collection);
  }
