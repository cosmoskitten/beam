...
public class TFRecordIO {
  ...
  public abstract static class Read extends PTransform<PBegin, PCollection<byte[]>> {
     ...
     * mode, the compression type of the file is determined by its extension (e.g., {@code *.gz} is
     * gzipped, {@code *.zlib} is zlib compressed, and all other extensions are uncompressed).
     */
    ...
    protected FileBasedSource<byte[]> getSource() {
      return CompressedSource.wrap(new TFRecordSource(getFilepattern()), getCompression());
    }
