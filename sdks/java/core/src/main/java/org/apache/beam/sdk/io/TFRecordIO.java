...
public class TFRecordIO {
    ...
    static class TFRecordReader extends FileBasedReader<byte[]> {
      ...
      protected boolean readNextRecord() throws IOException {
        ...
        if (currentValue != null) {
          ...
          startOfRecord += codec.recordLength(currentValue);
          return true;
        } else {
  ...
  static class TFRecordSink extends FileBasedSink<byte[]> {
    ...
    private static class TFRecordWriter extends FileBasedWriter<byte[]> {
      ...
      protected void writeFooter() throws Exception {
        // Close here because there is currently no other natural place to do this. [BEAM-1465]
        outChannel.close();
      }
    }
  }
  ...
  private static class TFRecordCodec {
    private static int HEADER_LEN = (Long.SIZE + Integer.SIZE) / Byte.SIZE;
    private static int FOOTER_LEN = Integer.SIZE / Byte.SIZE;
    ...
    public byte[] read(ReadableByteChannel inChannel) throws IOException {
      ...
      checkState(
          ...
          "Invalided masked CRC32 of length");
