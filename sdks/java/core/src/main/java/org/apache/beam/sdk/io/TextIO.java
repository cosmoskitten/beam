      ...
      SerializableFunction<String, Boolean> isSplittable =
          new SerializableFunction<String, Boolean>() {
            ...
            public Boolean apply(String filename) {
              return getCompressionType() == CompressionType.UNCOMPRESSED
                  || (getCompressionType() == CompressionType.AUTO
                      && !CompressedSource.CompressionMode.isCompressed(filename));
            }
          };
      SerializableFunction<String, FileBasedSource<String>> createSource =
          new SerializableFunction<String, FileBasedSource<String>>() {
            @Override
            public FileBasedSource<String> apply(String input) {
              return TextIO.Read.wrapWithCompression(
                  new TextSource(StaticValueProvider.of(input)), getCompressionType());
            }
          };
      return input
          .apply(
              ...
              new ReadAllViaFileBasedSource<>(
                  isSplittable, getDesiredBundleSizeBytes(), createSource))
