 ...
 * can be bounded or unbounded - in either case, writing happens by default per window, and the
 * amount of data in each window is finite, so a finite number of files ("shards") are written for
 * each window. There are several aspects to this process:
 *
 * <ul>
 *   <li><b>How many shards are generated per window:</b> This is controlled by <i>sharding</i>,
 *       using {@link Write#withNumShards} or {@link Write#withSharding}. By default, every bundle
 *       in the input {@link PCollection} will produce a shard, so the number of shards will vary
 *       based on runner behavior, though at least 1 shard will always be produced for every
 *       non-empty window. Note that setting a fixed number of shards can hurt performance: it adds
 *       an additional {@link GroupByKey} to the pipeline.
 *   <li><b>How the shards are named:</b> This is controlled by a {@link Write.FilenamePolicy}:
 *       filenames can depend on a variety of inputs, e.g. the window, total number of shards and
 *       the current file's shard index, etc. - see {@link Write.FilenameContext}. The standard
 *       policies are {@link Write#prefixAndWindowedShard} and {@link
 *       Write#prefixAndUnwindowedShard}. A concise way to create a custom policy is {@link
 *       Write#prefixAndShardTemplate}.
 ...
 * destination, and {@link Write#to(SerializableFunction)} and the {@link
 * Write#via(SerializableFunction)} will be applied separately within each group. Note that
 * currently sharding can not be destination-dependent.
 *
 ...
 * destination. In that case, specify an <i>output function</i> in {@link
 * Write#via(SerializableFunction, SerializableFunction)} or {@link Write#via(SerializableFunction,
 * Sink)}.
 *
...
public class FileIO {
  ...
  public static <InputT> Write<Void, InputT> write() {
    return FileIO.<Void, InputT>writeDynamic()
        .by(SerializableFunctions.<InputT, Void>constant(null))
  ...
  public abstract static class Write<DestinationT, UserT>
     ...
     * </ul>
     */
    public static FilenamePolicy prefixAndShardTemplate(
    ...
    /**
     * Generates filenames as {@link #prefixAndShardTemplate(String, String)} with a shard template
     * of {@link DefaultFilenamePolicy#DEFAULT_WINDOWED_SHARD_TEMPLATE}, which is {@code
     * W-P-SSSSS-of-NNNNN}.
     */
    public static FilenamePolicy prefixAndWindowedShard(final String prefix) {
      return prefixAndShardTemplate(
     ...
     * Generates filenames as {@link #prefixAndShardTemplate(String, String)} with a shard template
     * of {@link DefaultFilenamePolicy#DEFAULT_WINDOWED_SHARD_TEMPLATE}, which is {@code
     * SSSSS-of-NNNNN} and ignores the window and pane. Note that this should be used only if the
     * input is globally windowed with a default (or at-most-once) trigger.
     */
    public static FilenamePolicy prefixAndUnwindowedShard(final String prefix) {
      return prefixAndShardTemplate(
          prefix, DefaultFilenamePolicy.DEFAULT_UNWINDOWED_SHARD_TEMPLATE);
    }

    @Nullable
    abstract SerializableFunction<DestinationT, Sink<?>> getSinkFn();

    @Nullable
    abstract SerializableFunction<UserT, ?> getOutputFn();
    ...
    abstract static class Builder<DestinationT, UserT> {
      abstract Builder<DestinationT, UserT> setSinkFn(
          SerializableFunction<DestinationT, Sink<?>> sink);

      abstract Builder<DestinationT, UserT> setOutputFn(
          SerializableFunction<UserT, ?> outputFn);
    ...
    public <OutputT> Write<DestinationT, UserT> via(
        SerializableFunction<UserT, OutputT> outputFn,
        SerializableFunction<DestinationT, Sink<OutputT>> sinkFn) {
      checkArgument(sinkFn != null, "sinkFn can not be null");
      checkArgument(outputFn != null, "outputFn can not be null");
      return toBuilder().setSinkFn((SerializableFunction) sinkFn).setOutputFn(outputFn).build();
    }

    /**
     * Like {@link #via(SerializableFunction, SerializableFunction)}, but uses the same sink for all
     * destinations.
     */
    public <OutputT> Write<DestinationT, UserT> via(
        SerializableFunction<UserT, OutputT> outputFn, Sink<OutputT> sink) {
      checkArgument(sink != null, "sink can not be null");
      checkArgument(outputFn != null, "outputFn can not be null");
      return via(outputFn, SerializableFunctions.<DestinationT, Sink<OutputT>>constant(sink));
    }

    /**
     * Like {@link #via(SerializableFunction, SerializableFunction)}, but the output type of the
     * sink is the same as the type of the input collection.
     */
    public Write<DestinationT, UserT> via(SerializableFunction<DestinationT, Sink<UserT>> sinkFn) {
      checkArgument(sinkFn != null, "sinkFn can not be null");
      return toBuilder()
          .setSinkFn((SerializableFunction) sinkFn)
          .setOutputFn(SerializableFunctions.<UserT>identity())
          .build();
    }

    /**
     * Like {@link #via(SerializableFunction)}, but uses the same {@link Sink} for all destinations.
     */
    public Write<DestinationT, UserT> via(Sink<UserT> sink) {
      checkArgument(sink != null, "sink can not be null");
      return via(SerializableFunctions.<DestinationT, Sink<UserT>>constant(sink));
    }
    ...
    /**
     * Like {@link #to(SerializableFunction)} but uses the same {@link FilenamePolicy} for all
     * destinations.
     */
    public Write<DestinationT, UserT> to(FilenamePolicy policy) {
      checkArgument(policy != null, "policy can not be null");
      return to(SerializableFunctions.<DestinationT, FilenamePolicy>constant(policy));
    }
    ...
    /**
     * Specifies the side inputs that should be made available to all user code running as part of
     * this transform.
     */
    public Write<DestinationT, UserT> withSideInputs(List<PCollectionView<?>> sideInputs) {
      checkArgument(sideInputs != null, "sideInputs can not be null");
      return toBuilder().setSideInputs(sideInputs).build();
    }

    /** Like {@link #withSideInputs(List)}. */
    ...
    private static class ViaFileBasedSink<UserT, DestinationT, OutputT>
      ...
      private final SerializableFunction<DestinationT, Sink<OutputT>> sinkFn;
      ...
      private ViaFileBasedSink(
        ...
        super(
            ...
            spec.getCompression());
        this.sinkFn = (SerializableFunction) spec.getSinkFn();
      }
          ...
          public Writer<DestinationT, OutputT> createWriter() throws Exception {
            return new Writer<DestinationT, OutputT>(this, "") {
              ...
              protected void prepareWrite(WritableByteChannel channel) throws Exception {
                sink = sinkFn.apply(getDestination());
                sink.open(channel);
              }
      ...
      private static class DynamicDestinationsAdapter<UserT, DestinationT, OutputT>
        ...
        private final SerializableFunction<UserT, DestinationT> destinationFn;
        private final SerializableFunction<UserT, OutputT> outputFn;
        private final SerializableFunction<DestinationT, Write.FilenamePolicy> filenamePolicyFn;
        ...
        private DynamicDestinationsAdapter(
            Write<DestinationT, UserT> spec, Coder<DestinationT> destinationCoder) {
          this.destinationFn = spec.getDestinationFn();
          this.outputFn = ((SerializableFunction<UserT, OutputT>) spec.getOutputFn());
          this.filenamePolicyFn = spec.getFilenamePolicyFn();
          this.emptyWindowDestination = spec.getEmptyWindowDestination();
          this.destinationCoder = destinationCoder;
          this.compression = spec.getCompression();
          this.sideInputs = spec.getSideInputs();
        }

        @Override
        public OutputT formatRecord(UserT record) {
          return outputFn.apply(record);
        }

        @Override
        public DestinationT getDestination(UserT element) {
          return destinationFn.apply(element);
        }
        ...
        public FilenamePolicy getFilenamePolicy(final DestinationT destination) {
          final Write.FilenamePolicy policyFn = filenamePolicyFn.apply(destination);
          return new FilenamePolicy() {
            @Override
