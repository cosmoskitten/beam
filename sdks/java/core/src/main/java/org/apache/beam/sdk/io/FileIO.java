 ...
 *
 * <h4>Example: Watching a single filepattern for new files</h4>
 *
 ...
 * }</pre>
 *
 * <h4>Example: Matching a PCollection of filepatterns arriving from Kafka</h4>
 *
 ...
 *
 * <h4>Example: Returning filenames and contents of compressed files matching a filepattern</h4>
 *
 ...
 *       control over which elements go into which files, consider using "dynamic destinations" (see
 *       below).
 ...
 *   <li><b>How all of the above can be element-dependent:</b> This is controlled by "dynamic
 *       destinations". It is possible to have different groups of elements use different policies
 ...
 *
 * <h4>Example: Writing CSV files</h4>
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <h4>Example: Writing CSV files to different directories and with different headers</h4>
 *
 * <pre>{@code
 ...
 * transactions.apply(MapElements
 *         .into(lists(strings()))
 *         .via(tx -> tx.getType().getAllFields(tx))))
 *     .apply(FileIO.<TransactionType, List<String>>writeDynamic()
 *         .by(tx -> tx.getType())
 *         .via(type -> new CSVSink(type.getFieldNames()))
...
public class FileIO {
  ...
  public abstract static class Write<DestinationT, UserT>
    ...
    @Nullable
    abstract SerializableFunction<DestinationT, Sink<UserT>> getSinkFn();
    ...
    abstract static class Builder<DestinationT, UserT> {
      abstract Builder<DestinationT, UserT> setSinkFn(
          SerializableFunction<DestinationT, Sink<UserT>> sink);
    ...
    /** Specifies how create a {@link Sink} for a particular destination. */
    public Write<DestinationT, UserT> via(
        SerializableFunction<DestinationT, Sink<UserT>> sinkFn) {
      checkArgument(sinkFn != null, "sinkFn can not be null");
      return toBuilder().setSinkFn(sinkFn).build();
    }

    /**
     * Like {@link #via(SerializableFunction)} but uses the same {@link Sink} for all
     * destinations.
     */
    ...
    private static class ViaFileBasedSink<UserT, DestinationT>
      ...
      private final SerializableFunction<DestinationT, Sink<UserT>> sinkFn;
