...
class WatermarkHold<W extends BoundedWindow> implements Serializable {
   ...
   * <li>A timer will be set (by {@link ReduceFnRunner}) to clear the hold by the end of the window.
   * In other words the input watermark cannot be ahead of the end of the window.
   * </ol>
   ...
   * (ie the end of window time), or {@literal null} if no hold was added. We only add the hold if
   * we can be sure a timer will be set (by {@link ReduceFnRunner}) to clear it. In other words, the
   * input watermark cannot be ahead of the end of window time.
   ...
   * <li>We guarantee an {@link Timing#ON_TIME} pane will be emitted for all windows (except the
   * {@link GlobalWindow}, see below) which saw at least one element, even if that
   * {@link Timing#ON_TIME} pane is empty. Thus when elements in a pane are processed due to a
   * fired trigger we must set both an end of window timer and an end of window hold. Again, the
   * hold ensures the {@link Timing#ON_TIME} pane will not be considered late by any downstream
   * computation.
   * </ol>
   * <p>If we are in the {@link GlobalWindow} then case 1 above still holds since during a
   * pipeline drain the {@code ON_TIME} pane should be emitted if it is non-empty. However we
   * declare case 2 is not supported.
  ...
  private Instant addEndOfWindowHold(ReduceFn<?, ?, ?, W>.Context context, boolean paneIsEmpty) {
    ...
    if (eowHold.isBefore(inputWM)) {
      which = "too late for end-of-window timer";
      added = false;
    } else if (paneIsEmpty && !eowHold.isBefore(GlobalWindow.INSTANCE.maxTimestamp())) {
      which = "unnecessary (empty, global window)";
      added = false;
    } else {
      which = "on time";
      added = true;
    ...
    WindowTracing.trace(
        ...
        + "key:{}; window:{}; inputWatermark:{}; outputWatermark:{}",
        eowHold, which, context.key(), context.window(), inputWM, outputWM);
   ...
   * which the hold was added (ie the end of window time plus allowed lateness), or
   * {@literal null} if no hold was added. We only add the hold if it is distinct from
   * what would be added by {@link #addEndOfWindowHold}. In other words,
   * {@link WindowingStrategy#getAllowedLateness} must be non-zero.
   ...
   * <li>The {@link WindowingStrategy#getClosingBehavior()} is {@link ClosingBehavior#FIRE_ALWAYS},
   * and thus we guarantee a final pane will be emitted for all windows (except
   * the {@link GlobalWindow}, see below) which saw at least one element. Again, the garbage
   * collection hold guarantees that any empty final pane can be given a timestamp which will
   * not be considered beyond allowed lateness by any downstream computation.
   * </ol>
   * <p>If we are in the {@link GlobalWindow} then case 1 still applies so that panes will
   * fire during drain. However we declare case 2 not supported.
   * <p>We use {@code paneIsEmpty} to distinguish cases 1 and 2.
   */
  @Nullable
  private Instant addGarbageCollectionHold(
    ...
    if (eow.isBefore(GlobalWindow.INSTANCE.maxTimestamp())) {
      gcHold = gcHold.plus(windowingStrategy.getAllowedLateness());
    }
    if (!windowingStrategy.getAllowedLateness().isLongerThan(Duration.ZERO)) {
      which = "unnecessary (no allowed lateness)";
      added = false;
    } else if (paneIsEmpty && context.windowingStrategy().getClosingBehavior()
                              == ClosingBehavior.FIRE_IF_NON_EMPTY) {
      which = "unnecessary (empty, FIRE_IF_NON_EMPTY)";
      added = false;
    } else if (paneIsEmpty && !eow.isBefore(GlobalWindow.INSTANCE.maxTimestamp())) {
      which = "unnecessary (empty, global window)";
      added = false;
    } else {
      which = "on time";
