...
public class FileSystems {
  ...
  public static void rename(
    ...
    getFileSystemInternal(srcToRename.iterator().next().getScheme()).rename(srcToRename, destToRename);
  }
    ...
    if (Sets.newHashSet(moveOptions).contains(MoveOptions.StandardMoveOptions.IGNORE_MISSING_FILES)) {
      resourceIdsToDelete = FluentIterable
          ...
          .transformAndConcat(new Function<MatchResult, Iterable<Metadata>>() {
            @Override
            public Iterable<Metadata> apply(MatchResult input) {
              try {
          ...
          .transform(new Function<Metadata, ResourceId>() {
            @Override
            public ResourceId apply(Metadata input) {
