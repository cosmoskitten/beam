...
public class ApiSurface {
  ...
  /**
   * Allows matching classes against an {@link ApiSurface}.
   * Aims at keeping the public API is conformant to a hard-coded policy by controlling what
   * classes are allowed to be exposed by an API surface.
   */
  ...
  private static class SurfaceMatcher extends TypeSafeDiagnosingMatcher<ApiSurface> {
    ...
    private boolean verifyNoAbandoned(final ApiSurface checkedApiSurface,
      ...
      final Sets.SetView<Matcher<Class<?>>> abandonedPackages =
          Sets.difference(allowedClasses, matchedPackages);
  ...
  public static Matcher<Class<?>> classesInPackage(final String packageName) {
    return new SurfaceMatcher.ClassInPackage(packageName);
  }
