...
public final class ByteKeyRangesTracker extends AbstractByteKeyRangeTracker {
  ...
  @Override
  protected synchronized void setEndPosition(ByteKey end) {
  ...
  @Override
  protected synchronized void setStartingPosition(ByteKey start) {
  ...
  @Override
  protected boolean containsKey(ByteKey key) {
  ...
  public synchronized double getFractionConsumed() {
    ...
    for (ByteKeyRange range : ranges) {
      ...
      if (position.compareTo(range.getEndKey()) >= 0) {
        consumed = consumed.add(rangeSize);
        ...
        consumed = consumed.add(positionValue.subtract(rangeStartValue));
      } else {
        break;
      }
    }
    ...
    // double.
    BigInteger consumedScaled = consumed.shiftLeft(64);
    return consumedScaled.divide(rangesSize).doubleValue() / Math.pow(2, 64);
  }
  ...
  public synchronized ByteKey interpolateKey(double fraction) {
    ...
    // If we're executing outside of the loop, that means that remainingTotalSize has had every
    // range in ranges subtracted from it, which means when remainingTotalSize was initialized, it
    // was almost equal to totalRangesSize. This implies fraction was extremely close to 1. Thus,
  ...
  /**
   * Assumes {@code ranges} are non-overlapping.
   */
