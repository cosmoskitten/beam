...
public class PCollectionList<T> implements PInput, POutput {
   ...
   * arbitrary tag associated with each.
   */
  ...
  public boolean equals(Object other) {
    ...
    if (!this.pipeline.equals(that.pipeline)
        || this.pcollections.size() != that.pcollections.size()) {
      return false;
    }
    for (int i = 0; i < this.pcollections.size(); i++) {
      // Tags in a PCollectionList are arbitrary. Two PCollections with equivalent PCollections in
      // an identical order are equal.
      if (!this.pcollections.get(i).equals(that.pcollections.get(i))) {
        return false;
      }
    }
    return true;
