...
public class Watch {
  ...
  public abstract static class Growth<InputT, OutputT, KeyT>
    ...
    abstract static class Builder<InputT, OutputT, KeyT> {
      ...
      abstract Builder<InputT, OutputT, KeyT> setKeyCoder(Coder<KeyT> keyCoder);
    ...
    public Growth<InputT, OutputT, KeyT> withKeyCoder(Coder<KeyT> keyCoder) {
    ...
    public PCollection<KV<InputT, OutputT>> expand(PCollection<InputT> input) {
      ...
      if (getOutputKeyFn() == null) {
        ...
        if (keyCoder == null) {
          ...
          try {
            keyCoder = input.getPipeline().getCoderRegistry().getCoder(keyT);
          } catch (CannotProvideCoderException e) {
            throw new RuntimeException(
                "Unable to infer coder for KeyT ("
                    + keyT
                    + "). Specify it explicitly using withKeyCoder().");
          }
        }
        try {
          ...
          throw new IllegalArgumentException(
              "Key coder " + keyCoder + " must be deterministic");
        }
      }

      return input
          .apply(ParDo.of(new WatchGrowthFn<>(this, outputCoder, outputKeyFn, keyCoder))
  ...
  private static class WatchGrowthFn<InputT, OutputT, KeyT, TerminationStateT>
    ...
    public GrowthTracker<OutputT, KeyT, TerminationStateT> newTracker(
      ...
      return new GrowthTracker<>(
          outputKeyFn, keyCoder, restriction, getTerminationCondition());
    }
      ...
      this.coderFunnel =
          new Funnel<OutputT>() {
            ...
            public void funnel(OutputT from, PrimitiveSink into) {
              try {
                keyCoder.encode(
                    keyFn.apply(from), Funnels.asOutputStream(into));
              } catch (IOException e) {
