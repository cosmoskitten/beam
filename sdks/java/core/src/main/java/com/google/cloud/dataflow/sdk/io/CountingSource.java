...
public class CountingSource {
  ...
  private static class UnboundedCountingReader extends UnboundedReader<Long> {
    ...
    private long current;
    private long elementsRemainingThisPeriod;
    ...
    public UnboundedCountingReader(UnboundedCountingSource source, CounterMark mark) {
      ...
      if (mark == null) {
        ...
        elementsRemainingThisPeriod = source.elementsPerPeriod;
        this.lastTimeProduced = Instant.now();
      } else {
        this.current = mark.getLastEmitted();
        this.elementsRemainingThisPeriod = mark.getElementsRemainingThisPeriod();
    ...
    public boolean advance() throws IOException {
      ...
      elementsRemainingThisPeriod--;
      if (elementsRemainingThisPeriod == 0) {
    ...
    public CounterMark getCheckpointMark() {
      return new CounterMark(current, elementsRemainingThisPeriod, lastTimeProduced);
    }
  ...
  public static class CounterMark implements UnboundedSource.CheckpointMark {
    ...
    private final long elementsRemainingThisPeriod;
    private final Instant lastProduced;

    /**
     * Creates a checkpoint mark reflecting the last emitted value.
     */
    public CounterMark(long lastEmitted, long elementsRemainingThisPeriod, Instant lastProduced) {
      this.lastEmitted = lastEmitted;
      this.elementsRemainingThisPeriod = elementsRemainingThisPeriod;
      this.lastProduced = lastProduced;
    }

    public long getElementsRemainingThisPeriod() {
      return elementsRemainingThisPeriod;
    }
    ...
    private CounterMark() {
      this.lastEmitted = 0L;
      this.elementsRemainingThisPeriod = 0L;
