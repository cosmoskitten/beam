...
/**
 * Maps windows using a window mapping fn. The input is a window, the output must be a
 * {@link KV} with the key being the original window and the value being the mapped window.
 */
public class WindowMappingFnRunner {
  ...
  static class Factory<InputT extends BoundedWindow, OutputT extends BoundedWindow>
      extends MapRunner.Factory<InputT, KV<InputT, OutputT>> {

    @Override
    public ThrowingFunction<InputT, KV<InputT, OutputT>> createMapFunctionForPTransform(
      ...
      return (InputT input) -> KV.of(input, windowMappingFn.getSideInputWindow(input));
    }
  }
}
