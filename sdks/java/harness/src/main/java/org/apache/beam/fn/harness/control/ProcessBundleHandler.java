...
import org.apache.beam.fn.harness.control.ProcessBundleHandler.BundleExecutionController.ExecutionState;
...
public class ProcessBundleHandler {
  ...
  private final ConcurrentMap<String, BundleExecutor> instructionIdToSplitHandlers;
  ...
  ProcessBundleHandler(
    ...
    instructionIdToSplitHandlers = new ConcurrentHashMap<>();
  }
  ...
  private void createRunnerAndConsumersForPTransformRecursively(
      ...
      org.apache.beam.fn.harness.control.BundleExecutionController splitListener)
      throws IOException {
  ...
  public BeamFnApi.InstructionResponse.Builder processBundle(BeamFnApi.InstructionRequest request)
    ...
    try (HandleStateCallsForBundle beamFnStateClient =
      ...
      try {
        instructionIdToSplitHandlers.put(request.getInstructionId(), bundleExecutor);
        ...
        // TODO: Check whether we checkpointed all element processing because we are in
        // the middle of a split request. If we are currently splitting, finalize
        // the split handler's current split request and resume processing
        // any remaining primaries.
        //
        // readThread.processElements(...)
        // if(activeSplit) {
        //   complete split future
        //   resumeProcessing
        //
        ...
        /**
         * Eagerly remove the {@link BundleExecutor} to prevent a race condition where we
         * are attempting to send back the {@link ProcessBundleResponse} and are receiving split
         * requests before the runner knows that the bundle is done.
         */
        instructionIdToSplitHandlers.remove(request.getInstructionId());

        /**
         * Transition the {@link BundleExecutor} to a finished state preventing any
         * remaining and known inflight requests to be rejected.
         */
        ...
        /**
         * Ensure whether we completed exceptionally or normally that the {@link
         * BundleExecutor} is removed closing a race condition where we are attempting to
         * send back the {@link ProcessBundleResponse} and are receiving split requests before the
         * runner knows that the bundle is done.
         */
        instructionIdToSplitHandlers.remove(request.getInstructionId());
      }
    }
  ...
  private static class BundleExecutor {
          ...
          case CHECKPOINTING:
            BeamFnApi.ProcessBundleSplitResponse response =
                BeamFnApi.ProcessBundleSplitResponse.newBuilder()
                    .addAllPrimaryRoots(primaryRoots)
                    .addAllResidualRoots(residualRoots)
                    .build();
            primaryRoots.clear();
            residualRoots.clear();
            activeSplit = null;
    ...
    private class ExecutionController implements BundleExecutionController {
      ...
      public void checkpoint(
          BeamFnApi.BundleApplication primary, BeamFnApi.DelayedBundleApplication residual) {
  ...
  public BeamFnApi.InstructionResponse.Builder splitBundle(BeamFnApi.InstructionRequest request)
    ...
    BundleExecutor handler =
        instructionIdToSplitHandlers.get(splitRequest.getInstructionReference());
    if (handler == null) {
      throw new Exception(
          String.format(
