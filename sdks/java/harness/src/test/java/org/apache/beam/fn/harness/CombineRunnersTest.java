...
public class CombineRunnersTest {
  private static class TestCombineFn extends CombineFn<Integer, Double, String> {
  ...
  public void createPipeline() throws Exception {
    ...
    Combine.PerKey<String, Integer, String> combine = Combine.perKey(combineFn);

    Pipeline p = Pipeline.create();
    PCollection<KV<String, Integer>> inputPCollection = p.apply(Create.of(KV.of("unused", 0)));
    inputPCollection.setCoder(KvCoder.of(StringUtf8Coder.of(), BigEndianIntegerCoder.of()));
    PCollection<KV<String, String>> outputPCollection = inputPCollection
        .apply(TEST_COMBINE_ID, combine);
    outputPCollection.setCoder(KvCoder.of(StringUtf8Coder.of(), StringUtf8Coder.of()));
  ...
  public void testPrecombine() throws Exception {
    ...
    Deque<WindowedValue<KV<String, Double>>> mainOutputValues = new ArrayDeque<>();
    consumers.put(Iterables.getOnlyElement(pTransform.getOutputsMap().values()),
        (FnDataReceiver) (FnDataReceiver<WindowedValue<KV<String, Double>>>) mainOutputValues::add);
    ...
    assertThat(sum, closeTo(9.0, 0.001));

    assertThat(
        mainOutputValues,
        contains(
            valueInGlobalWindow(KV.of("B", 2.0)),
            valueInGlobalWindow(KV.of("C", 3.0))));
  }
  ...
  public void testMergeAccumulators() throws Exception {
    ...
    FnDataReceiver<WindowedValue<?>> input =
        Iterables.getOnlyElement(consumers.get(inputPCollectionId));
    input.accept(valueInGlobalWindow(KV.of("A", Arrays.asList(1.0, 2.0, 6.0))));
    input.accept(valueInGlobalWindow(KV.of("B", Arrays.asList(2.0, 3.0))));
    input.accept(valueInGlobalWindow(KV.of("C", Arrays.asList(5.0, 2.5))));

    assertThat(
        mainOutputValues,
        contains(
            valueInGlobalWindow(KV.of("A", 9.0)),
            valueInGlobalWindow(KV.of("B", 5.0)),
            valueInGlobalWindow(KV.of("C", 7.5))));
  }
  ...
  public void testExtractOutputs() throws Exception {
    // Create a map of consumers and an output target to check output values.
    Multimap<String, FnDataReceiver<WindowedValue<?>>> consumers = HashMultimap.create();
    Deque<WindowedValue<KV<String, String>>> mainOutputValues = new ArrayDeque<>();
    consumers.put(Iterables.getOnlyElement(pTransform.getOutputsMap().values()),
        (FnDataReceiver) (FnDataReceiver<WindowedValue<KV<String, String>>>) mainOutputValues::add);
    ...
    FnDataReceiver<WindowedValue<?>> input =
        Iterables.getOnlyElement(consumers.get(inputPCollectionId));
    input.accept(valueInGlobalWindow(KV.of("A", 9.0)));
    input.accept(valueInGlobalWindow(KV.of("B", 5.0)));
    input.accept(valueInGlobalWindow(KV.of("C", 7.5)));

    assertThat(
        mainOutputValues,
        contains(
            valueInGlobalWindow(KV.of("A", String.valueOf(9.0))),
            valueInGlobalWindow(KV.of("B", String.valueOf(5.0))),
            valueInGlobalWindow(KV.of("C", String.valueOf(7.5)))));
  }
  ...
  public void testCombineGroupedValues() throws Exception {
    ...
    assertThat(
        ...
        contains(
            valueInGlobalWindow(KV.of("A", String.valueOf(9.0))),
            valueInGlobalWindow(KV.of("B", String.valueOf(5.0))),
            valueInGlobalWindow(KV.of("C", String.valueOf(7.0)))));
  }
}
