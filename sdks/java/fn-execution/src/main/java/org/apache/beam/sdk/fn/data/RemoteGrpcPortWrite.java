 ...
 */
public class RemoteGrpcPortWrite {
   ...
   */
  public static RemoteGrpcPortWrite writeToPort(String localInputId, RemoteGrpcPort port) {
    return new RemoteGrpcPortWrite(localInputId, port);
  }
  ...
  private final String localInputId;
  private final RemoteGrpcPort port;

  private RemoteGrpcPortWrite(String localInputId, RemoteGrpcPort port) {
    this.localInputId = localInputId;
    this.port = port;
  }

  public RemoteGrpcPort getPort() {
    return port;
  }
  ...
  public PTransform toPTransform() {
    return PTransform.newBuilder()
        ...
        .putInputs(LOCAL_INPUT_ID, localInputId)
        .build();
  }

  @Override
  public boolean equals(Object other) {
    if (!(other instanceof RemoteGrpcPortWrite)) {
      return false;
    }
    RemoteGrpcPortWrite that = (RemoteGrpcPortWrite) other;
    return Objects.equals(this.localInputId, that.localInputId)
        && Objects.equals(this.port, that.port);
  }

  @Override
  public int hashCode() {
    return Objects.hash(localInputId, port);
  }
}
