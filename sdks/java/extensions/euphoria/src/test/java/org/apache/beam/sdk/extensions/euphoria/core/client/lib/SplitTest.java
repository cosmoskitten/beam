...
public class SplitTest {
  ...
  public void testBuild() {
    ...
    final PCollection<String> dataset = OperatorTests.createMockDataset(TypeDescriptors.strings());

    final Split.Output<String> split =
        Split.named(opName).of(dataset).using((UnaryPredicate<String>) what -> true).output();

    final Filter positive = (Filter) OperatorTests.getProducer(split.positive());
    assertNotNull(positive.getPredicate());
    final Filter negative = (Filter) OperatorTests.getProducer(split.negative());
  ...
  public void testBuild_NegatedPredicate() {
    final PCollection<Integer> dataset =
        OperatorTests.createMockDataset(TypeDescriptors.integers());
    final Split.Output<Integer> split =
        Split.of(dataset).using((UnaryPredicate<Integer>) what -> what % 2 == 0).output();

    final Filter<Integer> oddNumbers = (Filter) OperatorTests.getProducer(split.negative());
