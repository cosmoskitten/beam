...
public class MockedUnboundedTable extends MockedTable {
  ...
  private MockedUnboundedTable(BeamRowSqlType beamSqlRowType) {
  ...
  public MockedUnboundedTable addRows(Duration duration, Object... args) {
    List<BeamRow> rows = TestUtils.buildRows(getRowType(), Arrays.asList(args));
  ...
  @Override public PCollection<BeamRow> buildIOReader(Pipeline pipeline) {
    TestStream.Builder<BeamRow> values = TestStream.create(beamRowSqlType.getRecordCoder());
