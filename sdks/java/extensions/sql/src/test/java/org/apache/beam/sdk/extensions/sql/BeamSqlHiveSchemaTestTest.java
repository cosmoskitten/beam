...
public class BeamSqlHiveSchemaTestTest {
  ...
  @Test
  public void testSelectFromHive() {
    PCollection<Row> inputMain =
        pipeline.apply("mainInput", create(row(1, "pcollection_1"), row(2, "pcollection_2")));

    TableProvider hive = hiveTableProvider();

    PCollection<Row> result =
        inputMain.apply(
            SqlTransform.query("SELECT * FROM `hive`.`default`.`a_csv`")
                .withTableProvider("hive", hive));
        ...
        inputMain.apply(
            SqlTransform.query(
                "SELECT extra.f_int, main.f_string || extra.f_string AS f_string \n"
                    ...
                    + " beam.PCOLLECTION AS main \n"
                    + "   ON main.f_int = extra.f_int"));
    //                    .withDefaultTableProvider("hiveSchema", hive));
  ...
  private TableProvider hiveTableProvider() {
    return HCatalogTableProvider.create(
        // metastore.thrift.uris
        ImmutableMap.of("hive.metastore.uris", "thrift://....:9083"));
  }
