...
public class JoinReorderingTest {
  ...
  public void testSystemNotReorderingWithoutRules() {
    TestTableProvider tableProvider = new TestTableProvider();
    createThreeTables(tableProvider);
    ImmutableList.Builder<RelOptRule> ruleSetBuilder = ImmutableList.<RelOptRule>builder();
    for (RuleSet set : BeamRuleSets.getRuleSets()) {
      for (RelOptRule rule : set) {
        if (!(rule instanceof BeamJoinPushThroughJoinRule)
            && !(rule instanceof BeamJoinAssociateRule)
            && !(rule instanceof JoinCommuteRule)) {
    ...
    BeamSqlEnv env =
        BeamSqlEnv.builder(tableProvider)
            ...
            .setRuleSets(new RuleSet[] {RuleSets.ofList(ruleSetBuilder.build())})
            .build();
