...
public class BeamSqlNotExpressionTest extends BeamSqlFnExecutorTestBase {
  @Test public void evaluate() throws Exception {
    ...
    Assert.assertTrue(new BeamSqlNotExpression(operands).evaluate(row, null, ImmutableMap.of()).getBoolean());

    operands.clear();
    operands.add(BeamSqlPrimitive.of(SqlTypeName.BOOLEAN, true));
    Assert.assertFalse(new BeamSqlNotExpression(operands).evaluate(row, null, ImmutableMap.of()).getBoolean());

    operands.clear();
    operands.add(BeamSqlPrimitive.of(SqlTypeName.BOOLEAN, null));
    Assert.assertNull(new BeamSqlNotExpression(operands).evaluate(row, null, ImmutableMap.of()).getValue());
  }
}
