...
public class CustomTableResolverTest implements Serializable {
  ...
  public void testDefaultBuildIOReader_withEmptyParams_returnsPCollection() {
    ...
    PCollection<Row> resultWithNull = testTable.buildIOReader(pipeline.begin(), null, null);
    PCollection<Row> resultWithEmpty =
        testTable.buildIOReader(
            pipeline.begin(), new DefaultTableFilter(ImmutableList.of()), ImmutableList.of());

    PAssert.that(resultWithNull).containsInAnyOrder(expected);
  ...
  public void testDefaultBuildIOReader_withNonEmptyParams_throwsException() {
    ...
    assertThrows(
        ...
        () -> testTable.buildIOReader(pipeline.begin(), () -> null, null));
    assertThrows(
        IllegalArgumentException.class,
        () -> testTable.buildIOReader(pipeline.begin(), null, ImmutableList.of("one")));
