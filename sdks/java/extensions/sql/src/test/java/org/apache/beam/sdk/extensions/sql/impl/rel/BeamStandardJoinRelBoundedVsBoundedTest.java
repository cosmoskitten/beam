...
public class BeamStandardJoinRelBoundedVsBoundedTest extends BaseRelTest {
  ...
  public void testNodeStatsEstimation() {
    ...
    while (!(root instanceof BeamStandardJoinRel)) {
    ...
    NodeStats leftEstimate =
        BeamSqlRelUtils.getNodeStats(
            ((BeamStandardJoinRel) root).getLeft(), root.getCluster().getMetadataQuery());
    NodeStats rightEstimate =
        BeamSqlRelUtils.getNodeStats(
            ((BeamStandardJoinRel) root).getRight(), root.getCluster().getMetadataQuery());
  ...
  public void testNodeStatsOfMoreConditions() {
    ...
    while (!(root1 instanceof BeamStandardJoinRel)) {
      root1 = root1.getInput(0);
    }

    RelNode root2 = env.parseQuery(sql2);

    while (!(root2 instanceof BeamStandardJoinRel)) {
