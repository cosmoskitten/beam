...
import static org.apache.beam.sdk.extensions.sql.meta.provider.test.TestTableProvider.PUSH_DOWN;
import static org.apache.beam.sdk.extensions.sql.meta.provider.test.TestTableProvider.PushDownOptions.PROJECT;
...
public class TestTableProviderTest {
  ...
  public void testInMemoryTableProviderWithProjectPushDown() {
    ...
    Table table = getTable("tableName", PROJECT);
  ...
  private static Table getTable(String name, PushDownOptions options) {
    return Table.builder()
        ...
        .comment(name + " table")
        .schema(BASIC_SCHEMA)
        .properties(JSON.parseObject("{ " + PUSH_DOWN + ": " + "\"" + options.toString() + "\" }"))
        .type("test")
        .build();
  }
}
