...
public class HCatalogTableProvider implements TableProvider, Serializable {
  ...
  public Table getTable(String name) {
    try {
      // tables should be looked up from sub-providers.
      // if we reached this then getSubProvider(name) returned null
      // meaning there's no such DB. Try to look up the table in the default DB
      // then.
      org.apache.hadoop.hive.metastore.api.Table metastoreTable =
          metastore().getTable("default", name);
      return TableUtils.toBeamTable(metastoreTable);
