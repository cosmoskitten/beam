...
/**
 * Utility methods to help wire up schema inferring using {@link InferredSqlRowCoder}.
 */
class SchemaHelper {
  ...
  static PCollection<Row> toRows(PInput input) {
    ...
    if (coder instanceof InferredSqlRowCoder) {
      InferredSqlRowCoder inferredSchemaCoder = (InferredSqlRowCoder) coder;
      return
          pCollection
              .apply(
                  ((PCollection<?>) input).getName() + "_transformToRows",
                  transformToRows(inferredSchemaCoder))
              .setCoder(inferredSchemaCoder.getRowCoder());
    }
