...
public class JdbcDriver extends Driver {
  ...
  private Properties ensureDefaultProperties(Properties originalInfo) {
    ...
    setIfNull(info, SCHEMA_FACTORY, BeamCalciteSchemaFactory.LoadAllProviders.class.getName());
   ...
   * specified here doesn't load any providers automatically. We then override the top-level "beam"
   * schema with {@code tableProvider}.
   *
   * <p>This is called in tests and {@link BeamSqlEnv}, core part of {@link SqlTransform}. CLI uses
   * standard JDBC driver registry, and goes through {@link #connect(String, Properties)}. That path
   * ends up using schema factory that populates the default schema with all table providers it can
   * find. See {@link BeamCalciteSchemaFactory}.
   */
  public static JdbcConnection connect(TableProvider tableProvider) {
    try {
      Properties properties = new Properties();
      setIfNull(properties, SCHEMA_FACTORY, BeamCalciteSchemaFactory.EmptySchema.class.getName());
