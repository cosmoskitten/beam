...
abstract class HllCountInitFn<InputT, HllT>
  ...
  // Result of an empty aggregation is represented by an byte[] of length 0, because we cannot
  // create sketches without knowing the type of its input data. An empty byte[] can be encoded by
  // the default ByteArrayCoder, and is more space-efficient and safer than using null. As opposed
  // to returning an empty PCollection, it allows us to return 0 when extracting from the sketch.
  @Override
