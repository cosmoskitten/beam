 ...
 * {@code KV<byte[], byte[]>}. A fraction of the generated records {@code KV<byte[], byte[]>} are
 * associated with "hot" keys, which are uniformly distributed over a fixed number of hot keys.
 * The remaining generated records are associated with "random" keys.
 * Each record will be slowed down by a certain sleep time generated based on the specified sleep
...
public class SyntheticBoundedInput {
  ...
  public static class SyntheticBoundedSource extends OffsetBasedSource<KV<byte[], byte[]>> {
    ...
    public final SyntheticBoundedSource createSourceForSubrange(long start, long end) {
      ...
      checkArgument(
          ...
          "End offset value " + end + " of the subrange cannot be larger than the end offset value "
              + getEndOffset() + " of the parent source");
   ...
   * in the {@link SyntheticBoundedSource}.
   */
  ...
  public static class SourceOptions extends SyntheticOptions {
     ...
     * 0 means the source is not dynamically splittable (but is perfectly statically splittable).
     * In that case it also doesn't report progress at all.
     */
    ...
    /**
     * If specified, this source will split into exactly this many bundles regardless of
     * the hints provided by the service.
     */
     ...
     * independent of the per-record delay and uses the same types of distributions as
     * {@link #delayDistribution}.
     */
    @JsonDeserialize(using = SamplerDeserializer.class)
    public Sampler initializeDelayDistribution = fromRealDistribution(
        new ConstantRealDistribution(0));
  ...
  /**
   * A reader over the {@link PCollection} of {@code KV<byte[], byte[]>}
  ...
  private static class SyntheticSourceReader
    ...
    protected boolean advanceImpl() throws IOException {
      ...
      SyntheticUtils.delay(record.sleepMsec, options.cpuUtilizationInMixedDelay,
