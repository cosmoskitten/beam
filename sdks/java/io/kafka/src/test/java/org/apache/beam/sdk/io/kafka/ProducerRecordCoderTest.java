...
public class ProducerRecordCoderTest {
  ...
  public void testProducerRecordStructuralValueWithHeadersApi() throws IOException {
    ...
    ProducerRecord<byte[], byte[]> producerRecord =
            new ProducerRecord<>("topic", 1, null, "key".getBytes(UTF_8), "value".getBytes(UTF_8), headers);
  ...
  @Test
  public void testProducerRecordStructuralValueWithoutHeaders() throws IOException {
    RecordHeaders headers = new RecordHeaders();
    headers.add("headerKey", "headerVal".getBytes(UTF_8));
    ProducerRecordCoder producerRecordCoder =
            ProducerRecordCoder.of(ByteArrayCoder.of(), ByteArrayCoder.of());
    ProducerRecord<byte[], byte[]> producerRecord =
            new ProducerRecord<>("topic", 1, null, "key".getBytes(UTF_8), "value".getBytes(UTF_8), headers);
    Whitebox.setInternalState(ConsumerSpEL.class, "hasHeaders", false);
    ProducerRecord testProducerRecord = (ProducerRecord) producerRecordCoder.structuralValue(producerRecord);
