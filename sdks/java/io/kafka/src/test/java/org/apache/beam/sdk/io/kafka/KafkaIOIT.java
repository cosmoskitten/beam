...
public class KafkaIOIT {
  ...
  /**
   * Map of hash for 1000 uniformly distributed records with 10B keys and 90B values (100kB total).
   */
  ...
  private KafkaIO.Write<byte[], byte[]> writeToKafka() {
    return KafkaIO.<byte[], byte[]>write()
        ...
        .withProducerConfigUpdates(
            ImmutableMap.of("retention.ms", Integer.MAX_VALUE, "log.retention.bytes", -1))
  ...
  private KafkaIO.Read<byte[], byte[]> readFromKafka() {
    return KafkaIO.readBytes()
        ...
        .withTopics(Lists.newArrayList(options.getKafkaTopic()))
  ...
  public interface Options extends PipelineOptions, StreamingOptions {
    ...
    @Description("Filename prefix for file with hashcode")
    @Validation.Required
    String getHashcodeFilenamePrefix();

    void setHashcodeFilenamePrefix(String path);
