...
public interface TimestampPolicyFactory<KeyT, ValueT> extends Serializable {
   ...
   * 'current time - 2 seconds'. If the partition is idle and idleWaterMarkDelta is not present then the
   * watermark does not advance.
   * See {@link KafkaIO.Read#withLogAppendTime()} for longer description.
   */
  static <K, V> TimestampPolicyFactory<K, V> withLogAppendTime(Optional<Duration> idleWaterMarkDelta) {
    return (tp, previousWatermark) -> new LogAppendTimePolicy<>(previousWatermark, idleWaterMarkDelta);
  }
  ...
  static <K, V> TimestampPolicyFactory<K, V> withLogAppendTime() {
    return (tp, previousWatermark) -> new LogAppendTimePolicy<>(previousWatermark, Optional.of(Duration.standardSeconds(2)));
  }
  ...
  class LogAppendTimePolicy<K, V> extends TimestampPolicy<K, V> {
     ...
     * which could ensure minimum LogAppendTime for future records.
     * We can either advance the watermark to 'last backlog check time - small delta to account for any internal
     * buffering in Kafka', or if set to an empty optional the user must ensure periodic heartbeats
     * on each partition of the topic.
     */
    private final Optional<Duration> idleWatermarkDelta;

    protected Instant currentWatermark;

    public LogAppendTimePolicy(Optional<Instant> previousWatermark, Optional<Duration> idleWatermarkDelta) {
