...
class KafkaUnboundedReader<K, V> extends UnboundedReader<KafkaRecord<K, V>> {
  ...
  public boolean advance() throws IOException {
    ...
    while (true) {
      if (curBatch.hasNext()) {
        ...
        curTimestamp = pState.timestampPolicy.getTimestampForRecord(record);
        curRecord = record;
  ...
  private static class PartitionState<K, V> {
    ...
    synchronized Instant updateAndGetWatermark() {
      lastWatermark = timestampPolicy.getWatermark(
        new TimestampPolicy.PartitionContext(backlogMessageCount(), latestOffsetFetchTime));
