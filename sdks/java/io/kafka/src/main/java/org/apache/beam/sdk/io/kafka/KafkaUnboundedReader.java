...
class KafkaUnboundedReader<K, V> extends UnboundedReader<KafkaRecord<K, V>> {
  ...
  private void updateLatestOffsets() {
    try {
      final Map<TopicPartition, PartitionState> topicPartitions = partitionStates.stream()
      ...
      newOffsets.forEach((tp, offset) -> topicPartitions.get(tp).setLatestOffset(offset, fetchTime));
    } catch (Exception e) {
