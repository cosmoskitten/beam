...
public class DatastoreV1 {
  ...
  public abstract static class Read extends PTransform<PBegin, PCollection<Entity>> {
    ...
    static Query translateGqlQueryWithLimitCheck(String gql, Datastore datastore,
      ...
      // The only way convert a gql query string to a Query is to run the query and extract it
      // from the response. To prevent reading any data we set the limit to 0 but if the user
      // query already has a limit set, we catch the exception and retry without the limit. This
      // will result in reading some data but should not be a large number since the service has
      // a cap on the number of entities returned for a single rpc request.
