...
public class DatastoreV1 {
  ...
  public abstract static class Read extends PTransform<PBegin, PCollection<Entity>> {
     ...
     *
     * Currently, the only way to translate a gql query string to a Query is to run the query
     ...
     *
     * Note: This may result in reading actual data from Cloud Datastore but the service has a cap
     * on the number of entities returned for a single rpc request, so this should not be a problem
     * in practice.
     */
    @VisibleForTesting
