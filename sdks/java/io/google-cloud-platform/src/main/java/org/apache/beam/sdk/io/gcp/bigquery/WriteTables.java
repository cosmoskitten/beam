 ...
 *
 * <p>The input is a list of files corresponding to each partition of a table. These files are load
 * into a temporary table (or into the final table if there is only one partition). The output is a
 * {@link KV} mapping each final table to a list of the temporary tables containing its data.
 *
 ...
 */
class WriteTables
    extends DoFn<KV<ShardedKey<TableDestination>, List<String>>, KV<TableDestination, String>> {
