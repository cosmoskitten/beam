 ...
 *
 * <p>Tables have rows ({@link TableRow}) and each row has cells ({@link TableCell}).
 * A table has a schema ({@link TableSchema}), which in turn describes the schema of each
 * cell ({@link TableFieldSchema}). The terms field and cell are used interchangeably.</p>
 *
 * <p>{@link TableSchema}: describes the schema (types and order) for values in each row.
 * It has one attribute, ‘fields’, which is list of {@link TableFieldSchema} objects.</p>
 *
 * {@link TableFieldSchema}: describes the schema (type, name) for one field.
 * It has several attributes, including 'name' and 'type'. Common values for
 * the type attribute are: 'STRING', 'INTEGER', 'FLOAT', 'BOOLEAN', 'NUMERIC', 'GEOGRAPHY'.
 * All possible values are described at:
 * <a href="https://cloud.google.com/bigquery/docs/reference/standard-sql/data-types">
 * https://cloud.google.com/bigquery/docs/reference/standard-sql/data-types</a></p>
 *
 * <p>{@link TableRow}: Holds all values in a table row. Has one attribute, 'f', which is a
 * list of {@link TableCell} instances.</p>
 *
 * <p>{@link TableCell}: Holds the value for one cell (or field). Has one attribute,
 * 'v', which is the value of the table cell.
 *
 ...
 * https://en.wikipedia.org/wiki/Well-known_text</a>) format for reading and writing
 * to BigQuery.
 * The BYTES data type requires that bytes are encoded using base64 encoding when writing
 * to BigQuery. When bytes are read from BigQuery they are returned as base64 encoded bytes.</p>
 *
 * <h3>Reading</h3>
 *
