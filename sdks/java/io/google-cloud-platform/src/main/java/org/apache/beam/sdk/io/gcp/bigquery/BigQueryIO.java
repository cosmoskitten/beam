...
public class BigQueryIO {
  ...
  public abstract static class Write<T> extends PTransform<PCollection<T>, WriteResult> {
     ...
     * {@link #withClusteringEnabled()} is also set so that the clustering configurations get
     ...
     * used when {@link#withTimePartitioning(TimePartitioning)} is set. If {@link #to(SerializableFunction)} or
     * {@link #to(DynamicDestinations)} is used to write to dynamic tables, this setting is ignored;
     * instead, set {@link #withClusteringEnabled()} and specify clustering per table in the
     * returned {@link TableDestination} objects.
     */
     ...
     */
    public Write<T> withClusteringEnabled() {
    ...
    public WriteResult expand(PCollection<T> input) {
      ...
      if (dynamicDestinations == null) {
        if (getJsonTableRef() != null) {
          ...
          dynamicDestinations = new TableFunctionDestinations<>(getTableFunction(), getEnableClustering());
        }
