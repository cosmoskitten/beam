...
public class BigQueryIOTest implements Serializable {
  ...
  public void testRemoveTemporaryTables() throws Exception {
    String projectId = "someproject";
    String datasetId = "somedataset";
    List<String> tableIds = Lists.newArrayList("table1", "table2", "table3");
    List<TableReference> tableRefs = Lists.newArrayList(
        BigQueryIO.parseTableSpec(String.format("%s:%s.%s", projectId, datasetId, tableIds.get(0))),
        BigQueryIO.parseTableSpec(String.format("%s:%s.%s", projectId, datasetId, tableIds.get(1))),
        BigQueryIO.parseTableSpec(String.format("%s:%s.%s", projectId, datasetId, tableIds.get(2))));

    doThrow(new IOException("Unable to delete table"))
        .when(mockDatasetService).deleteTable(projectId, datasetId, tableIds.get(0));
    doNothing().when(mockDatasetService).deleteTable(projectId, datasetId, tableIds.get(1));
    doNothing().when(mockDatasetService).deleteTable(projectId, datasetId, tableIds.get(2));
