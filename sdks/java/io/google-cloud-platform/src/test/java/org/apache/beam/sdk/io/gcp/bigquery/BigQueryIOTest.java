...
public class BigQueryIOTest implements Serializable {
  ...
  private static class FakeJobService implements JobService, Serializable {
    ...
    private void startJob(JobReference jobRef, GenericJson config) throws IOException, InterruptedException {
      if (!Strings.isNullOrEmpty(executingProject)) {
        checkArgument(
            jobRef.getProjectId().equals(executingProject),
            "Project id: %s is not equal to executing project: %s",
  ...
  public void testReadFromTable() throws IOException {
    ...
    SerializableFunction<GenericJson, Void> onStartJob =
        new SerializableFunction<GenericJson, Void>() {
          @Override
          public Void apply(GenericJson input) {
            ...
            for (String destination : destinations) {
                  ...
                  Schema.createRecord(
                      ...
                      ImmutableList.of(
                          ...
                          new Field(
                              "number",
                              Schema.createUnion(
                                  Schema.create(Type.LONG), Schema.create(Type.NULL)),
                              "The number field",
                              0)));
              try (WritableByteChannel channel = IOChannelUtils.create(newDest, MimeTypes.BINARY);
                ...
                Record aRec =
                    new GenericRecordBuilder(schema).set("name", "a").set("number", 1L).build();
                Record bRec =
                    new GenericRecordBuilder(schema).set("name", "b").set("number", 2L).build();
                Record cRec =
                    new GenericRecordBuilder(schema).set("name", "c").set("number", 3L).build();
                tableRowWriter.append(aRec);
                tableRowWriter.append(bRec);
                tableRowWriter.append(cRec);
              } catch (IOException e) {
                throw new IllegalStateException(
                    String.format("Could not create destination for extract job %s", destination),
                    e);
              }
            }
            return null;
