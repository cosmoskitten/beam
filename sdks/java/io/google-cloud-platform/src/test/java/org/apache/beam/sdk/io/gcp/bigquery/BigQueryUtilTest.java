...
public class BigQueryUtilTest {
  ...
  public void testWriteAppend() throws Exception {
    when(mockDatasetService.getTable(anyString(), anyString(), anyString()))
        .thenReturn(basicTableSchema());
    TableReference ref = BigQueryIO
        .parseTableSpec("project:dataset.table");

    BigQueryUtil bqUtil = new BigQueryUtil(mockDatasetService);
    bqUtil.getOrCreateTable(ref, BigQueryIO.Write.WriteDisposition.WRITE_APPEND,
  ...
  public void testWriteEmpty() throws Exception {
    ...
    bqUtil.getOrCreateTable(ref, BigQueryIO.Write.WriteDisposition.WRITE_EMPTY,
        BigQueryIO.Write.CreateDisposition.CREATE_NEVER, null);
  ...
  @Test
  public void testWriteEmptyFail() throws Exception {
    when(mockDatasetService.getTable(anyString(), anyString(), anyString()))
        .thenReturn(basicTableSchema());
    when(mockDatasetService.isTableEmpty(anyString(), anyString(), anyString()))
        .thenReturn(false);
