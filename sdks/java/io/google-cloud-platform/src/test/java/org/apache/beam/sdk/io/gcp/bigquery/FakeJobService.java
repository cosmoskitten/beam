...
class FakeJobService implements JobService, Serializable {
    ...
    synchronized (allJobs) {
      ...
      if (loadConfig.getSourceUris().size() > 0) {
        ...
        for (String file: loadConfig.getSourceUris()) {
          sourceFiles.add(FileSystems.matchNewResource(file, /* is directory */ false));
        }
        for (String file: loadFiles) {
          destinationFiles.add(FileSystems.matchNewResource(file, /* is directory */ false));
        }
  ...
  private void writeRowsHelper(List<TableRow> rows, Schema avroSchema,
    ...
    try (WritableByteChannel channel = FileSystems.create(
        FileSystems.matchNewResource(filename, /* is directory */ false), MimeTypes.BINARY);
