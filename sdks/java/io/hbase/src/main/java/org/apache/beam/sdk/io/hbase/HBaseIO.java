    ...
    public long getEstimatedSizeBytes(PipelineOptions pipelineOptions) throws Exception {
      if (estimatedSizeBytes == null) {
        try (Connection connection =
          ...
          estimatedSizeBytes =
              HBaseUtils.estimateSizeBytes(connection, read.serializableScan.get(), read.tableId);
        }
        LOG.debug(
    ...
    public List<? extends BoundedSource<Result>> split(
      ...
      try (Connection connection = ConnectionFactory.createConnection(read.getConfiguration())) {
        List<HRegionLocation> regionLocations =
            HBaseUtils.getRegionLocations(connection, read.serializableScan.get(), read.tableId);
        ...
        List<ByteKeyRange> ranges =
            HBaseUtils.getRanges(
                regionLocations, read.serializableScan.get(), read.tableId);
        final int numSources = ranges.size();
        LOG.debug("Spliting into {} source(s)", numSources);
        if (numSources >= 1) {
          List<HBaseSource> sources = new ArrayList<>(ranges.size());
