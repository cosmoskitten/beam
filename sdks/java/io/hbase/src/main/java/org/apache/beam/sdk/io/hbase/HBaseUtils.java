...
class HBaseUtils {
  ...
  static long estimateSizeBytes(Connection connection, Scan scan, String tableId) throws Exception {
    // This code is based on RegionSizeCalculator in hbase-server
    long estimatedSizeBytes = 0L;
    List<HRegionLocation> regionLocations = getRegionLocations(connection, scan, tableId);
  ...
  static List<HRegionLocation> getRegionLocations(Connection connection, Scan scan, String tableId)
      throws Exception {
  ...
  static List<ByteKeyRange> getRanges(
      List<HRegionLocation> regionLocations, Scan scan, String tableId) {
    byte[] startRow = scan.getStartRow();
    byte[] stopRow = scan.getStopRow();
