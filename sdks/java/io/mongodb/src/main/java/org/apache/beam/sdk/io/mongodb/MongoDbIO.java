...
public class MongoDbIO {
  ...
  public abstract static class Read extends PTransform<PBegin, PCollection<Document>> {
    ...
    @Nullable
    abstract List<BsonDocument> aggregation();

    @Nullable
    abstract String documentIdStr();

    @Nullable
    abstract ObjectId documentId();
    ...
    abstract static class Builder {
      ...
      abstract Builder setAggregation(List<BsonDocument> aggregation);
    ...
    public Read withAggregation(List<BsonDocument> value) {
      return builder().setAggregation(value).build();
    }

    public Read withDocumentIdStr(String value) {
      return builder().setDocumentIdStr(value).build();
    }

    public Read withDocumentId(ObjectId value) {
  ...
  static class BoundedMongoDbSource extends BoundedSource<Document> {
    ...
    public List<BoundedSource<Document>> split(
      ...
      try (MongoClient mongoClient =
        ...
        if (!spec.aggregation().isEmpty() || splitKeys.size() < 1) {
    ...
    public boolean start() {
      ...
      cursor =
          QueryBuilder.create(mongoCollection)
              ...
              .withAggregation(spec.aggregation())
