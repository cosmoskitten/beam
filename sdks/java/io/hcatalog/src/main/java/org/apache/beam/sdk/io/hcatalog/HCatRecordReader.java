...
public class HCatRecordReader extends DoFn<HCatRecordReader.PartitionWrapper, HCatRecord> {
  ...
  private ReaderContext getReaderContext(PartitionWrapper wrapper, long desiredSplitCount) throws HCatException {
  ...
  private long getFileSizeForPartitions(PartitionWrapper partitionWrapper) throws Exception {
    ...
    for (Map.Entry<String, String> entry : partitionWrapper.readSpec.getConfigProperties().entrySet()) {
    ...
    try {
      ...
      Table table = HCatUtil.getTable(
          client, partitionWrapper.readSpec.getDatabase(), partitionWrapper.readSpec.getTable());
  ...
  public void processElement(
    ...
    final int indexOfPartitionColumnWithWaterMark = watermarkPartitionColumn.indexOf(watermarkPartitionColumn);
    final String partitionWaterMark = values.get(indexOfPartitionColumnWithWaterMark);

    final Instant partitionWatermarkTimeStamp = processContext.element().watermarkerFn.apply(partitionWaterMark);
    ...
    //Once we have read completely from partition, we can advance the watermark associated with the partition
