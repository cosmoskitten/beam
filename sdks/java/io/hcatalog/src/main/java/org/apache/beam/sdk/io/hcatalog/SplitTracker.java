...
public class SplitTracker extends RestrictionTracker<Split, Integer>
  ...
  public boolean tryClaim(Integer position) {
    checkArgument(lastAttemptedSplit == null || position > lastAttemptedSplit,
        "Trying to claim a split %s while last attempted was %s",
        position,
        lastAttemptedSplit);
    checkArgument(
        position >= split.getFrom(), "Trying to claim split %s before start of the range %s", position, split);
