...
public class PartitionRangeTracker extends RestrictionTracker<PartitionRange, Partition>
  ...
  public PartitionRange checkpoint() {
    ...
    this.range = new PartitionRange(
        ImmutableList.copyOf(forSort.subList(0, lastClaimedPartitionIndex + 1)), comparator, lastClaimedPartition);
    return new PartitionRange(ImmutableList.copyOf(unprocessedPartitions), comparator, lastClaimedPartition);
  }
  ...
  public Backlog getBacklog() {
    ...
    return Backlog.of(BigDecimal.valueOf(
        Math.max(range.getPartitions().size() - range.getPartitions().indexOf(lastAttemptedPartition), 0)));
  }
