...
public class HCatalogIOTest implements Serializable {
  ...
  public void testWriteThenUnboundedReadSuccess() throws Exception {
    ...
    final PCollection<HCatRecord> records = readAfterWritePipeline
        .apply(Create.of(buildReadRequest()))
        .apply(reader).setCoder((Coder) WritableCoder.of(DefaultHCatRecord.class));

    final PCollection<String> output = records.apply(
        ParDo.of(
            new DoFn<HCatRecord, String>() {
              @ProcessElement
              public void processElement(ProcessContext c) {
  ...
  private static List<PartitionPoller.ReadRequest> buildReadRequest() {
    ...
    final HCatalogIO.Read read = HCatalogIO.read()
        .withDatabase(TEST_DATABASE)
        .withTable(TEST_TABLE)
        .withConfigProperties(getConfigPropertiesAsMap(service.getHiveConf()));
  ...
  private static class PartitionComparator implements SerializableComparator<Partition> {
    @Override
  ...
  private static class WatermarkTimestampConverter implements SerializableFunction<String, Instant> {
  ...
  private void reCreateTestTableForUnboundedReads() throws CommandNeedRetryException {
    ...
    service.executeQuery("ALTER TABLE " + TEST_TABLE
