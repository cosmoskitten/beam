...
public class JdbcIOIT {
   ...
   *
   * This test does not attempt to validate the data - we do so in the read test. This does make it
   * harder to tell whether a test failed in the write or read phase, but the tests are much easier
   * to maintain (don't need any separate code to write test data for read tests to the database.)
   */
   ...
   *
   * When doing the validation, we wish to ensure that we both:
   ...
   *
   * We do not wish to generate and compare all of the expected values, so this method uses
