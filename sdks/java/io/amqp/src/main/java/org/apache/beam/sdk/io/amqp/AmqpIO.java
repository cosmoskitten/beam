...
public class AmqpIO {
  ...
  private static class UnboundedAmqpSource extends UnboundedSource<String, AmqpCheckpointMark> {
    ...
    @Override
    public List<UnboundedAmqpSource> generateInitialSplits(int desiredNumSplits,
  ...
  private static class UnboundedAmqpReader extends UnboundedSource.UnboundedReader<String> {
    ...
    public boolean advance() {
      ...
      if (messenger.incoming() > 0) {
        ...
        Instant messageCreationTime = new Instant(message.getCreationTime());
        if (messageCreationTime.isBefore(watermark)) {
          watermark = messageCreationTime;
        }
  ...
  public abstract static class Write extends PTransform<PCollection<String>, PDone> {
    ...
    private static class WriteFn extends DoFn<String, Void> {
      ...
      @Setup
      public void startMessenger() throws Exception {
        messenger = new MessengerImpl();
        messenger.start();
      }
      ...
      @Teardown
      public void stopMessenger() throws Exception {
        if (messenger != null) {
          messenger.stop();
        }
      }
