...
import org.apache.beam.sdk.nexmark.model.sql.ToBeamRow;
 ...
 */
public class SqlQuery3 extends PTransform<PCollection<Event>, PCollection<BeamRow>> {
  ...
  private static final BeamRowCoder OUTPUT_RECORD_CODER = createOutputRecordCoder();
  ...
  @Override
  public PCollection<BeamRow> expand(PCollection<Event> allEvents) {
    PCollection<Event> windowed = fixedWindows(allEvents);

    PCollection<BeamRow> auctions = filter(windowed, e -> e.newAuction != null, Auction.class);
    PCollection<BeamRow> people = filter(windowed, e -> e.newPerson != null, Person.class);
  ...
  private PCollectionTuple createStreamsTuple(
      PCollection<BeamRow> auctions,
      PCollection<BeamRow> people) {
  ...
  private PCollection<BeamRow> filter(
    ...
    return allEvents
        ...
        .apply(QUERY_NAME + ".ToRecords." + modelName, ToBeamRow.parDo())
        .setCoder(getRecordCoder(clazz));
  }

  private BeamRowCoder getRecordCoder(Class modelClass) {
    return ADAPTERS.get(modelClass).getRecordType().getRecordCoder();
  }

  private static BeamRowCoder createOutputRecordCoder() {
    BeamRowSqlType outputRecordType = BeamRowSqlType.builder()
