...
public class SqlQuery3 extends PTransform<PCollection<Event>, PCollection<BeamRecord>> {
  ...
  public PCollection<BeamRecord> expand(PCollection<Event> allEvents) {
    PCollection<Event> windowed = window(allEvents);

    PCollection<BeamRecord> auctions = filter(windowed, NexmarkQuery.IS_NEW_AUCTION, Auction.class);
    PCollection<BeamRecord> people = filter(windowed, NexmarkQuery.IS_NEW_PERSON, Person.class);
  ...
  private PCollection<Event> window(PCollection<Event> events) {
    return events.apply(
                Window.<Event>into(new GlobalWindows())
                    .triggering(Repeatedly.forever((AfterPane.elementCountAtLeast(30))))
  ...
  private PCollectionTuple createStreamsTuple(
    ...
    return PCollectionTuple
        .of(new TupleTag<BeamRecord>("Auction"), auctions)
        .and(new TupleTag<BeamRecord>("Person"), people);
  }
