...
public class SqlQuery2Test {
  ...
  private static final List<BeamRow> BIDS_EVEN_RECORDS = ImmutableList.of(
      newBidRecord(BIDS.get(1)),
      newBidRecord(BIDS.get(3)),
      newBidRecord(BIDS.get(5)),
      newBidRecord(BIDS.get(7)));

  private static final List<BeamRow> BIDS_EVERY_THIRD_RECORD = ImmutableList.of(
  ...
  private static BeamRow newBidRecord(Bid bid) {
    return new BeamRow(BID_ADAPTER.getRecordType(), BID_ADAPTER.getFieldsValues(bid));
  }
