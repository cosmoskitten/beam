...
<!--When using the Flink Runner you will create a jar file containing your job that
can be executed on a regular Flink cluster. -->
...
<span class="language-py">2. Start the Flink portable JobService endpoint: `./gradlew :beam-runners-flink_2.11-job-server:runShadow`
</span>

<span class="language-py">3. Submit the Python pipeline to the above endpoint by using the `PortableRunner` and `job_service` set to `localhost:8099`
</span>
...
<span class="language-py">1. Start Flink cluster (e.g. locally on `localhost:8081`)
</span>

<span class="language-py">2. Start JobService with Flink web service endpoint: `./gradlew :beam-runners-flink_2.11-job-server:runShadow -PflinkMasterUrl=localhost:8081`
</span>
