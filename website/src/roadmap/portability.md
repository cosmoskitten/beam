...
- `environment_type` determines where user code will be executed.
  ...
  - `PROCESS`: User code is executed by processes that are automatically started by the runner on
    each worker node.
  - `DOCKER` (default): User code is executed within a container. Containers can be built from
    source by running the command `./gradlew :sdks:python:container:py35:docker` or similar for
    `py2`, `py36`, or `py37`. (Starting with Beam 2.16, container images for the SDK harness will be
    published as part of the release process.) This option provides the most isolation between the
