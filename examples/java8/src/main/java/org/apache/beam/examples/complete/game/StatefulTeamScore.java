...
public class StatefulTeamScore extends LeaderBoard {
  ...
  private static Map<String, FieldInfo<KV<String, Integer>>> configureCompleteWindowedTableWrite() {
    ...
    tableConfigure.put(
        "processing_time",
        new WriteWindowedToBigQuery.FieldInfo<KV<String, Integer>>(
            "STRING", (c, w) -> GameConstants.fmt.print(Instant.now())));
   ...
   *   <li>Stateful {@link DoFn} can determine when to output based on the state. This allows only
  ...
  public static class UpdateTeamScoreFn
     ...
     * state, which is used to refer the state in @ProcessElement.
     *
     ...
     * score per team. Any other class that extend {@link org.apache.beam.sdk.state.State} can be
     * used.</p>
     *
     * <p>In order to store the value, the state must be encoded. Therefore, we provide a coder, in
     * this case the {@link VarIntCoder}. If the coder is not provided as StateSpecs.value(), Beam's
     * coder inference will try to provide a coder automatically.</p>
     */
    @StateId(TOTAL_SCORE)
    ...
    /**
     * To use a state cell, annotate a parameter with @StateId that matches the state declaration.
     * The type of the parameter should match the StateSpec type.
     */
    @ProcessElement
    public void processElement(
      ...
      // State is read and modified by using read(), write() methods.
