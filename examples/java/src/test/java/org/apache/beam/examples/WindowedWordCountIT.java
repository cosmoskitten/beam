...
public class WindowedWordCountIT {
  ...
  private static class WordCountsMatcher extends TypeSafeMatcher<PipelineResult>
    ...
    private final Map<String, Long> expectedWordCounts;
    private final ShardedFile outputFile;
    private Map<String, Long> actualCounts;

    public WordCountsMatcher(Map<String, Long> expectedWordCounts, ShardedFile outputFile) {
    ...
    public void describeTo(Description description) {
      containsInAnyOrder(expectedWordCounts.entrySet()).describeTo(description);
    }

    @Override
    public void describeMismatchSafely(PipelineResult pResult, Description description) {
      containsInAnyOrder(expectedWordCounts.entrySet())
          .describeMismatch(actualCounts.entrySet(), description);
    }
  }
}
