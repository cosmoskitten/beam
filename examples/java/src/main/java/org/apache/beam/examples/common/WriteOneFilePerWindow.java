...
public class WriteOneFilePerWindow extends PTransform<PCollection<String>, PDone> {
  ...
  public WriteOneFilePerWindow(final String outputDirectory) {
    this.outputDirectory = FileSystems.matchNewResource(outputDirectory, true /* isDirectory */);
  }

  public WriteOneFilePerWindow(final ResourceId outputDirectory) {
  ...
  public PDone expand(PCollection<String> input) {
    return input.apply(
        TextIO.Write
            .to(outputDirectory)
            .withFilenamePolicy(new PerWindowFiles("output"))
            .withWindowedWrites()
            .withNumShards(3));
  }
